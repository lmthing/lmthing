/**
 * Shape guard for utility-auditor's hooks — a bad trigger string or budget key ships silently
...
const SPACE_ID = 'utility-auditor';
...
test('every hook has a valid shape and stays inside this space', async () => {
  ...
  assert.ok(names.length > 0, 'utility-auditor ships at least the daily sweep hook');
...
test('the daily sweep triggers the auditor sweep action with a bounded budget', async () => {
  const hook = await importHook('daily-sweep');
  assert.equal(hook.type, 'cron');
  assert.equal(hook.daily, '05:45');
  assert.equal(hook.trigger, 'utility-auditor/auditor#sweep');
