...
for (const size of LLM_MODELS) {
  describe(`space-deploy: DeployManager [${size}]`, () => {
    it.skipIf(!hasKeys)('explains the deployment lifecycle', { timeout: 90_000 }, async () => {
      const { Session, AgentLoop, resolveModel } = await import('lmthing')

      const instruct = readFileSync(join(SPACE, 'agents/agent-deploy-manager/instruct.md'), 'utf-8')
      const session = new Session()
      const model = resolveModel(size)
      const loop = new AgentLoop({ session, model, modelId: size, instruct, maxTurns: 3 })

      await loop.handleMessage('I built a space locally and want to deploy it so others can use the agents via API. What are my options?')
  ...
  describe(`space-deploy: SpaceMonitor [${size}]`, () => {
    it.skipIf(!hasKeys)('reports how to check deployment status', { timeout: 90_000 }, async () => {
      const { Session, AgentLoop, resolveModel } = await import('lmthing')

      const instruct = readFileSync(join(SPACE, 'agents/agent-space-monitor/instruct.md'), 'utf-8')
      const session = new Session()
      const model = resolveModel(size)
      const loop = new AgentLoop({ session, model, modelId: size, instruct, maxTurns: 3 })

      await loop.handleMessage('My deployed space has been pending for 10 minutes. How do I check what is happening?')
