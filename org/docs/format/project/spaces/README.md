...
- Because a project app ships `spaces` as one of its template directories, installing the app materializes the whole `spaces/` tree into the pod alongside `database/ pages/ api/ hooks/ components/` (`sdk/org/libs/cli/src/server/routes/apps.ts#APP_TEMPLATE_DIRS`).
...
- They are what a project's cron / database / event [hooks](../hooks/README.md) `trigger`: a hook's declarative `trigger: 'space/agent#action'` string is split into `{spaceRef, agentSlug, action}` and delegated to that agent (`sdk/org/libs/cli/src/server/routes/hooks.ts#HookManager.runHeadless`, `:328`).
