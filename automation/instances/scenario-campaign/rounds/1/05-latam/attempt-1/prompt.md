# Scenario campaign — `05-latam` · round 1 (BASELINE + FIRST NEW ACTS)

This is the **first round** for `05-latam`. Two goals, in order:

**1. Reach a green baseline.**
- Read `sdk/org/scenarios/05-latam/scenario.md`. If `sdk/org/scenarios/05-latam/run.mjs` does **not** exist yet, scaffold it from
  `sdk/org/scenarios/_template/run.mjs` and implement the Acts already described in `sdk/org/scenarios/05-latam/scenario.md`
  (1:1 with its Acts table), reproducing the literal user flow. If it exists, read it and confirm its
  Acts still match `sdk/org/scenarios/05-latam/scenario.md`.
- If `sdk/org/scenarios/05-latam/scenario.md` itself is a stub (some scenarios only have a title/idea — check
...
- Choose them from the feature catalog below — capabilities `05-latam` does **not** yet cover
  — extending the scenario naturally (same persona, same growing project, realistic drift).
- Add each new Act to `sdk/org/scenarios/05-latam/scenario.md`'s Acts table (+ any new user stories / expectations) AND to
  `sdk/org/scenarios/05-latam/run.mjs` (1:1), keeping every hardening pattern. Run the FULL scenario (old + new) live,
...
- **Scenario:** `05-latam` (`sdk/org/scenarios/05-latam`)  ·  **Round:** 1 — BASELINE + FIRST NEW ACTS  ·  **Branch:** `main`
...
(there is no CI/deploy in this loop — see **Target** below). Scenario files live at `sdk/org/scenarios/05-latam`;
...
- **Set `SCENARIO_TARGET=local`** in the environment of every harness command (`provision.mjs`,
  `sdk/org/scenarios/05-latam/run.mjs`, any probe). With it set, `getUser` returns `{ pod:'http://localhost:8080', token:null }`
...
Extend `05-latam` with **additional Acts**, run the **whole scenario end-to-end against the
...
1. **Bring up the local pod + smoke.** `cd sdk/org/scenarios/harness && SCENARIO_TARGET=local node
   ...
   `SCENARIO_TARGET=local node provision.mjs 05-latam` (returns `{ pod:'http://localhost:8080',
...
Maintain the per-run progress log at **/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/1/05-latam/PROGRESS.md**.
