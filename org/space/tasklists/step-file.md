...
> UNVERIFIED: the claim that the built-in `fetch` global must also appear in `functions` (as the `02-fetch_each.md` example comment states). Searched `sdk/org/libs/core/src/exec/bootstrap.ts` — `fetch` is injected into EVERY child VM unconditionally via `createFetchGlobal` (`bootstrap.ts:159`) and is not part of the `pickAllowed` space-function set, so it is not stripped by the allowlist. Listing it (as the example does) is harmless but appears unnecessary for the built-in `fetch`; `webFetch`/`webSearch` gating IS grounded above.
...
Source: `store/projects/blog/spaces/newsroom/tasklists/refresh/01-load_sources.md:1-18` and `store/projects/blog/spaces/newsroom/tasklists/refresh/02-fetch_each.md:1-67`.

## Note on the `input` field

A step-level `input:` frontmatter map is parsed into `TaskNode.input` `sdk/org/libs/core/src/spaces/tasklist-load.ts:113-119`, but at fork time a task's actual inputs are composed from the seed-filtered tasklist input merged with upstream outputs (plus `item`/`index` for a `forEach` element) `sdk/org/libs/core/src/tasklist/orchestrator.ts:239` · `sdk/org/libs/core/src/tasklist/orchestrator.ts:300-301`.

> UNVERIFIED: whether a per-step `input:` schema is enforced/validated at runtime. Searched `sdk/org/libs/core/src/tasklist/*` and `sdk/org/libs/core/src/fork/fork.ts` for a consumer of `TaskNode.input` — found none; only the tasklist-level `index.md` `input` schema is validated (against the seed) by `validateInput` `sdk/org/libs/core/src/tasklist/orchestrator.ts:75-99`. The per-step `input:` appears declarative/documentation only.
