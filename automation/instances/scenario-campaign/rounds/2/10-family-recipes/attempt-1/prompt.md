# Scenario campaign — `10-family-recipes` · round 2 (ADD ACTS (batch 2))

`10-family-recipes` already has a runner and has been extended in earlier rounds. This round: **add
ANOTHER batch of additional Acts and continue from there.** Never regress the existing Acts —
extension is strictly additive.

1. **Orient & resume.** Read `sdk/org/scenarios/10-family-recipes/scenario.md` (esp. its Acts table + Actual-results), `sdk/org/scenarios/10-family-recipes/run.mjs`,
   and `sdk/org/scenarios/10-family-recipes/results/` (report, trace, checkpoint) to see exactly which Acts exist, which passed,
   and any open frontier a prior round recorded. Read this run's `/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/2/10-family-recipes/PROGRESS.md` if it exists.
2. **Pick the next batch of Acts (2–4)** from the feature catalog below — capabilities
   `10-family-recipes` still does **not** cover — extending the same persona / same growing project
...
3. **Add them to `sdk/org/scenarios/10-family-recipes/scenario.md` (Acts table + user stories/expectations) AND `sdk/org/scenarios/10-family-recipes/run.mjs` (1:1)**,
...
- **Scenario:** `10-family-recipes` (`sdk/org/scenarios/10-family-recipes`)  ·  **Round:** 2 — ADD ACTS (batch 2)  ·  **Branch:** `main`
...
1. **catalog-scout** — READ-ONLY prep: read sdk/org/scenarios/10-family-recipes/scenario.md + sdk/org/scenarios/10-family-recipes/run.mjs + sdk/org/scenarios/10-family-recipes/results and the feature catalog in this prompt; list which catalog capabilities 10-family-recipes does NOT yet cover, and propose the next 2–4 Acts (name + what each asserts on the trace/real state). Do NOT provision, open a THING session, or send any turn — analysis only.
...
testing a script. Hold them in `sdk/org/scenarios/10-family-recipes/scenario.md` and in every message `sdk/org/scenarios/10-family-recipes/run.mjs` sends.
...
A scenario is TWO files plus generated results, all under `sdk/org/scenarios/10-family-recipes`:
...
- **Every fixture must carry at least one fact/token that appears in NO other fixture**, and an Act
  ...
  assertion worthless.
- Fixtures are **committed and self-contained** under `sdk/org/scenarios/10-family-recipes/fixtures/`. Record where each
  came from (source URL) in `fixtures/links.md`.
- **Wire it into BOTH** `scenario.md` (the user flow + the Acts table) and `sdk/org/scenarios/10-family-recipes/run.mjs` (uploaded,
...
Pick additional Acts that exercise catalog capabilities `10-family-recipes` does **not** yet cover.
...
**Never exercised — pick 2–4 that fit `10-family-recipes`'s persona and build them into the story.** A
...
`10-family-recipes` — recovered errors stay a metric, unrecovered ones fail the run.
...
const user  = await getUser('10-family-recipes');            // { userId, token, pod }
...
const r = new Report('10-family-recipes', 'title');
r.step('Act VI — …', 'expected'); r.check('label', pass, actual); r.metric('turn', s, 's');
r.save('sdk/org/scenarios/10-family-recipes/results/report.md'); r.saveTrace('sdk/org/scenarios/10-family-recipes/results/trace.json', thing);
```
...
2. **Run Act by Act, in the FOREGROUND.** You are a **headless `claude -p` session**: when you stop
   ...
   until it exits, e.g. `cd sdk/org/scenarios/harness && node sdk/org/scenarios/10-family-recipes/run.mjs --acts=1` with the Bash
...
Maintain the per-run progress log at **/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/2/10-family-recipes/PROGRESS.md**.
...
- `sdk/org/scenarios/10-family-recipes/scenario.md` has all six sections + the feature checklist + the Acts table. **Round 1: do not
  rewrite the story** — it was authored deliberately; implement it. Later rounds: add the new Acts.
- `sdk/org/scenarios/10-family-recipes/run.mjs` **exists and implements `scenario.md` Act for Act (1:1)** — on round 1 you are writing
...
- **From round 2 on: at least one NEW real fixture** sourced from the real web (or Azure-TTS-generated
  for audio), committed under `fixtures/`, uploaded by `sdk/org/scenarios/10-family-recipes/run.mjs`, and asserted via a token that
