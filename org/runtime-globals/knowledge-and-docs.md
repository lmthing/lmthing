...
> UNVERIFIED — a **delegate** builds its `YieldRouterContext` without `knowledgeSpaceDir` (`sdk/org/libs/core/src/delegate/delegate.ts:340-355`, and `rg knowledgeSpaceDir` in `libs/core/src` hits only `fork.ts:431` and the router), and its `processYield` returns `undefined` for anything the router does not handle (`sdk/org/libs/core/src/delegate/delegate.ts:383`) — i.e. exactly the shape that produced the fork bug above. I found no test exercising `loadKnowledge` inside a delegate (`rg -l loadKnowledge` over `libs/core/**/*.test.ts` hits only fork/session/bootstrap/prelude tests), so whether a delegate binds the content (via the `vm.getVar` fallback in `bindYieldResults`, `sdk/org/libs/core/src/eval/turn-loop.ts:293-295`) or `undefined` is not established by the code I read.
