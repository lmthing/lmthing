...
`pnpm agents` (`package.json:10`) brings up the coding-agent fleet in a dedicated herdr workspace named `fleet`, driven by [`devops/scripts/start-agent-fleet.sh`](../../../devops/scripts/start-agent-fleet.sh). The workspace starts with a **single** pane — the `orchestrator` (claude opus, `--dangerously-skip-permissions`), seeded with [`.claude/agent-fleet.md`](../../../.claude/agent-fleet.md) as standing policy (`devops/scripts/start-agent-fleet.sh:36-42`, `:45-47`); no subagent panes exist yet (`devops/scripts/start-agent-fleet.sh:4-7`). The orchestrator spawns each one on demand with [`devops/scripts/spawn-agent.sh`](../../../devops/scripts/spawn-agent.sh) `<name>`, which splits a fresh pane, runs `herdr agent start`, and prints `<name> <pane-id>` on its final stdout line (`devops/scripts/spawn-agent.sh:107-116`). Requirements: the herdr TUI is already running (the script probes `herdr workspace list`; there is no detached server start — `devops/scripts/start-agent-fleet.sh:23-27`) and `herdr`, `claude`, `pi`, and `agy` are on `PATH` (`devops/scripts/start-agent-fleet.sh:18-21`; the agent kinds the roster launches, `devops/scripts/spawn-agent.sh:24-30`; the spawn helper repeats the same preflight, `devops/scripts/spawn-agent.sh:54-62`). The script is idempotent — it refuses to create a second `fleet` workspace, so re-running just tells you the fleet is up (`devops/scripts/start-agent-fleet.sh:29-33`).
