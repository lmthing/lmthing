...
- **`capabilities:`** — parsed into the project-app grant set (`db:*`/`views:write`/`api:write`/`hooks:write`/…) by `parseCapabilities` `sdk/org/libs/core/src/spaces/load.ts:468` · `sdk/org/libs/core/src/spaces/capabilities.ts` `parseCapabilities`.
