...
|---|---|---|---|
| `GET` | `/api/store/spaces` | `handleListStoreSpaces` `sdk/org/libs/cli/src/server/routes/store-spaces.ts#handleListStoreSpaces` | `200 { spaces: CatalogSpace[] }` (`{ spaces: [] }` when the store is unreachable) |
| `POST` | `/api/store/spaces/install` | `handleInstallStoreSpace` `sdk/org/libs/cli/src/server/routes/store-spaces.ts#handleInstallStoreSpace` | `200 { ok:true, projectId, spaceId }` · `200 { ok:false, diverged:true, … }` · `400` · `404` |
| `GET` | `/api/projects/:projectId/integrations` | `handleListProjectIntegrations` `sdk/org/libs/cli/src/server/routes/store-spaces.ts#handleListProjectIntegrations` | `200 { integrations: InstalledIntegration[] }` |
...
The route is a thin face over the **pure** engine `installStoreSpace(opts)` `sdk/org/libs/cli/src/server/routes/store-spaces.ts#installStoreSpace`, which returns a discriminated result the route maps to statuses `sdk/org/libs/cli/src/server/routes/store-spaces.ts#InstallStoreSpaceResult,333-346`.
...
- The global is created in `sdk/org/libs/core/src/globals/store.ts#createInstallSpaceGlobal` (`createInstallSpaceGlobal`, yield kind `installSpace`) and gated on the bare capability `store:install` at injection `sdk/org/libs/core/src/exec/bootstrap.ts:195-198` · `sdk/org/libs/core/src/spaces/capabilities.ts#CapabilityId`, with a matching DTS fragment (`STORE_INSTALL_DTS`, registered under `store:install` in `CAPABILITY_DTS_FRAGMENTS`) so an ungranted call fails typecheck `sdk/org/libs/core/src/typecheck/library-dts.ts:262-263,286`.
- The pod resolver is `createStoreResolver` `sdk/org/libs/cli/src/server/store-resolver.ts#createStoreResolver`, folded into `AppGlobalImpls.store` per project by the SessionManager `sdk/org/libs/cli/src/server/session-manager.ts#SessionManager.withStore`. It calls `installStoreSpace` **without `force`** — the agent path always respects the divergence guard; overwriting local edits stays a deliberate HTTP/UI action `sdk/org/libs/cli/src/server/store-resolver.ts:43-51`.
