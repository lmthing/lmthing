...
**Who may call it:** only a **write-capable fork role** (`sdk/org/libs/core/src/exec/capability.ts#forkCapabilities`). A top-level session (`sdk/org/libs/core/src/exec/capability.ts#sessionCapabilities`) and a delegate (`sdk/org/libs/core/src/exec/capability.ts#delegateCapabilities`) hold neither the injected global nor its declaration, so the call fails typecheck there. In the shipped spaces that is exactly where registration happens: the architect's `register` and `reregister` nodes, both `role: general` (`sdk/org/libs/core/system-spaces/system-architect/tasklists/synthesize_and_run/07-register.md`, `sdk/org/libs/core/system-spaces/system-architect/tasklists/iterate_space/04-reregister.md`).

Resolution has two sites, both writing the **same shared `dynamicSpaces` Map**:
- **Top-level session** (`sdk/org/libs/core/src/session/session.ts#Session.handleYield`): `loadSpace(dir)` → `this.dynamicSpaces.set(dir, space)`; on failure returns `{ ok:false, error }` rather than throwing. Retained but no longer reachable from a session's own VM, since sessions do not hold the capability — it is the resolver a fork's yield still routes through.
