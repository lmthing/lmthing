...
| `useParams` | `<T>() => T` | routing `…/runtime/router.tsx:89-91` |
| `Link` | `(props: { to?: string; href?: string } & AnchorHTMLAttributes) => ReactElement` | routing `…/runtime/router.tsx:143-162` |
| `navigate` | `(to: string) => void` | routing `…/runtime/router.tsx:121-124` |
| `mountApp` / `AppRoot` / `matchRoutes` | mount + match plumbing | used by the **generated entry**, not by page authors `…/runtime/router.tsx:217-222` |
...
Views must style with **design tokens only** — no hex, no literal `rgb()/hsl()`, no stock Tailwind color utilities (`gray-*`/`blue-*`/`green-500`); use `bg-background`, `text-foreground`, `bg-primary`, `border-border`, `var(--destructive)`, … (root `CLAUDE.md` design-system section; canonical spec `sdk/org/libs/css/DESIGN.md`). The runtime itself obeys it: `NotFound` is `text-muted-foreground p-4` `sdk/org/libs/cli/src/app/runtime/router.tsx:207-213` and `<Chat>` styles exclusively with `var(--primary)`, `var(--border)`, `var(--destructive)`, `var(--muted-foreground)` `sdk/org/libs/cli/src/app/runtime/chat.tsx:220-294`. Shipped pages follow suit (`store/projects/trips/pages/index.tsx:13-25`).
...
> UNVERIFIED: whether the token rule is CI-enforced for project-app views. `pnpm lint:tokens` scans `sdk/org/libs/css/src sdk/org/libs/ui/src sdk/org/apps/web/src com/src social/src team/src store/src space/src blog/src casa/src` (`package.json:14`) — that list does **not** include `store/projects/` or a pod project root, and I found no token-lint step inside the page build (searched `sdk/org/libs/cli/src/app/build/**`). For views the rule appears to be an authoring mandate, not an automated gate.
