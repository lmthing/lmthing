...
The **system spaces are reachable through the same routes**: `<root>/system/spaces/<id>` matches the generic `<root>/<projectId>/spaces/<id>` shape, so the reserved project id `system` (`SYSTEM_PROJECT_ID`) resolves and Studio browses/edits `system-global`, `user-thing`, … via `/api/projects/system/spaces/<id>/files` with no special-casing in the space routes `sdk/org/libs/cli/src/server/projects.ts:10-31`. It is an alias, not a project: `listProjects` never lists it `sdk/org/libs/cli/src/server/projects.ts#listProjects`, and it cannot be created or deleted `sdk/org/libs/cli/src/server/projects.ts:33-43`.
