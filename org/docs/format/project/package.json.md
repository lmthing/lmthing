...
- **`dependencies`** — the shared design system `@lmthing/ui` + `@lmthing/css` are pulled in as `workspace:*` (`store/projects/blog/package.json` `dependencies`). Pages style **only** with design tokens from `@lmthing/css` (see [`pages/view-spec.md`](./pages/view-spec.md)).
- **`@app/runtime` / `@app/types` are runtime-provided, not npm dependencies.** They never appear in `dependencies`; the whole-app typecheck declares `@app/runtime` as an in-memory ambient module and maps `@app/types` to the project's generated `types/generated.d.ts` (`sdk/org/libs/cli/src/app/build/typecheck.ts#buildAmbientDts`).
