...
| [views.md](./views.md) | Pages, the client router, `@app/runtime` hooks, `<Chat>` — **and** the `ViewRenderer`, which draws a spec page on the web bundle *and* natively on a phone |
...
Every project is a **served app from the moment it is created** — there is no "project without an app" state, and no separate app-launcher surface. A newborn project's app is a single full-height **chat page**, and as the conversation earns real structure the builder grows it into a multi-page app whose chat lives on as a floating dock. The `/chat` surface itself opens the project's chat — the full `ChatView` transcript talking to the project's top-level THING agent (`sdk/org/libs/ui/src/chat/app/session-control.ts#resolveProjectChat`) — while the project's **served app pages** are rendered by the shared `ViewRenderer` wherever they are opened (the native Home cover, a `GET /api/apps/:id/views` payload), reaching the pod because the host proxies `/app/*` to it (`devops/argocd/envoy/chat-routes.yaml`, the `chat-app-proxy` HTTPRoute). See [../chat/views.md](../chat/views.md#opening-a-projects-chat).
...
- **Security boundary = the pod, not the app.** Worker isolation of api handlers is a *crash* boundary (`sdk/org/libs/cli/src/app/api/runtime.ts:L6-L12`), never an auth boundary. A project-app is **single-user and carries no auth of its own**: the pod's HTTP server has no auth middleware at all — `createServer` hands every request straight to the `Router`, which matches on method + path only and has no token/JWT/session concept (`sdk/org/libs/cli/src/server/serve.ts:L343-L369`; `sdk/org/libs/cli/src/server/router.ts#Router.dispatch`), and neither `createAppApiHandler` nor `createPageServeHandler` reads an `Authorization` header or cookie.
  ...
  The *only* auth is the **platform picking which pod a request reaches**, and it lives at the Envoy edge, not in this codebase's app layer: the `app-jwt` `SecurityPolicy` validates the gateway-issued HS256 JWT — taken from the `Authorization: Bearer` header, an `access_token` query param, **or** an `access_token` cookie (page navigations and their `<script>`/`<link>` sub-requests cannot set a header) — and projects its `sub` claim into `x-user-id` (`devops/argocd/envoy/app-policies.yaml:L170-L221`). The policy is **`optional`**: a request with *no* token passes the filter (no `x-user-id` is set), but an *invalid* token is still rejected — so a forged token is blocked at the filter and only a token-less request is let through. The `app-lua-routing` Lua filter then routes by `x-user-id`: a **document** navigation with no `x-user-id` (a logged-out visitor typing `lmthing.app/<project>/`) is redirected `302 → /` to the shell, whose `AuthGate` sends them to sign in (rather than answering Envoy's raw JWT 401); a token-less **non-document** request (asset/XHR, including the app's own `/<project>/api/*` data calls) is `401`'d; an authenticated request is rewritten to that user's pod, `lmthing.user-<id>.svc.cluster.local:8080` (`devops/argocd/envoy/app-policies.yaml:L35-L127`). Locally there is no gateway and no auth at all.
