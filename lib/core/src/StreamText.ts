...
export class StreamTextBuilder {
    ...
    private _getMiddleware(): LanguageModelV3Middleware {
        return {
            ...
            wrapStream: async ({ doStream, params }) => {
              ...
              if (this._debugRunId) {
                this._debugStepNumber++; // Increment step number for each step
    ...
    protected _getModelString(): string {
        ...
        // Try to get model ID from the model object
        const model = this._model as any;
        if (model.modelId) {
            return model.modelId;
        }
        if (model.provider && model.provider === 'openai') {
            return `openai:${model.modelId || 'unknown'}`;
        }
        return 'custom-model';
    }
    ...
    protected async _logRequestFromParams(params: any): Promise<void> {
        ...
        // Extract system prompt from params if available
        const systemPrompt = params.system;

        // Extract messages from params
        const messages = params.prompt || [];

        // Extract tools from the current tools set
