...
the base URL to the user's **`DEMO_BASE_URL`** (e.g. a `https://webhook.site/<id>` URL) and attaches
their **`DEMO_API_TOKEN`** as an `Authorization: Bearer` header — the agent passes only a RELATIVE
...
user's `DEMO_WEBHOOK_SECRET`, then hands the raw payload to the Demo Channel agent, which replies via
