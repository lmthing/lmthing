...
**`COMMON_DTS`** — declarations present in **every** VM context (session, fork leaf, delegate): `display`, `inspect`, `loadKnowledge`, `sleep`, `registerSpace`, the `JSXDescriptor`/`ForkOpts`/`DelegateOpts` interfaces, the classic-JSX `React` and `JSX` namespaces, and host-injected primitives (`readDocument`, `integrationStatus`, `typecheckSource`, `spacePath`, `resolveSpaceDir`, `progress`), plus `catalogDts()` (the design-system catalog as typed JSX globals) appended at the end `library-dts.ts:35-107`. Note `readFileRaw` is NO LONGER in `COMMON_DTS` — it was split out into `READ_FILE_RAW_DTS` and is not emitted on any agent's model DTS (see the write-primitives note below). `fetch` and `process` are deliberately **not** in this list — see the internal-only-primitives paragraph below.
