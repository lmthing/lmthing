...
scope — both the parent repo and the `sdk/org` submodule. You **commit** your work on `{{branch}}`
(submodule first, then the parent pointer) so the ledger + artifacts are durable; pushing is optional
(there is no CI/deploy in this loop — see **Target** below). Scenario files live at `{{SCENARIO_DIR}}`;
...
- **Set `SCENARIO_TARGET=local`** in the environment of every harness command (`provision.mjs`,
  `{{RUN_MJS}}`, any probe). With it set, `getUser` returns `{ pod:'http://localhost:8080', token:null }`
...
Extend `{{SCENARIO_ID}}` with **additional Acts**, run the **whole scenario end-to-end against the
...
1. **Bring up the local pod + smoke.** `cd sdk/org/scenarios/harness && SCENARIO_TARGET=local node
   ...
   `SCENARIO_TARGET=local node provision.mjs {{SCENARIO_ID}}` (returns `{ pod:'http://localhost:8080',
...
- Both repos **committed** on `{{branch}}` (submodule first, then parent pointer); pushing is optional.
