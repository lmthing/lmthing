/**
 * Shape guard for utility-ledger's hooks — a bad trigger string or budget key ships silently
...
test('every hook has a valid shape and stays inside this space', async () => {
  ...
  assert.ok(names.length > 0, 'utility-ledger ships at least the monthly close hook');
...
test('the monthly close runs on a DAILY schedule and gates the day inside the tasklist', async () => {
  const hook = await importHook('monthly-close');
  assert.equal(hook.type, 'cron');
  assert.equal(hook.daily, '07:30');
  assert.equal(hook.trigger, 'utility-ledger/bookkeeper#close');
  assert.ok(hook.budget?.maxEpisodes > 0 && hook.budget?.maxWallClockMs > 0, 'the cron close must be budget-bounded');
  // Cron has no day-of-month field: the 1st-of-month gate MUST live in the close tasklist.
  const gate = await readFile(join(SPACE, 'tasklists', 'close', '01-gate.md'), 'utf8');
  assert.match(gate, /getUTCDate\(\) === 1/, 'the close tasklist must carry the 1st-of-month gate');
});
