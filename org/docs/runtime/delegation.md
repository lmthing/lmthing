...
- **`DelegateOpts = { query?, context?, attachmentIds? }`** (`sdk/org/libs/core/src/globals/delegate.ts#DelegateOpts`; DTS twin at `sdk/org/libs/core/src/typecheck/library-dts.ts:88-94`). `attachmentIds` are upload ids resolved to real bytes/notes by the ONE shared `resolveDelegateAttachments` helper before handing them to the child — images ride as a `MediaPart`, files become an id-anchored note telling the specialist to call `readDocument(id)` (`sdk/org/libs/core/src/session/session.ts#Session.resolveDelegateAttachments`). **Both delegate entry points call it** — the top-level model-initiated `runDelegate` yield context AND `runDelegateForFork` (every fork/tasklist-issued `delegate()` call) — so an attached image/file resolves identically whether the delegating agent is the top-level model or a task inside a tasklist. (Historically only the top-level path resolved `attachmentIds`; a fork/tasklist `delegate()` carrying one silently dropped it, since `delegate/delegate.ts` only ever reads the resolved `attachments`/`attachmentTexts`, never the raw ids — fixed by sharing this one resolver between the top-level `runDelegate` built in `sdk/org/libs/core/src/session/session.ts#Session.buildYieldContext` and `sdk/org/libs/core/src/session/session.ts#Session.runDelegateForFork`.)
- **A mismatched id throws, it does not silently drop.** The delegating agent has no copy-by-reference for an id — it retypes it by hand from the prose `attachmentNote()` puts on its message — so a single mistyped character used to hand the child ZERO attachment info, indistinguishable from "nothing was ever attached", with no signal to self-correct. `resolveDelegateAttachments` checks every requested id against `pendingAttachments` and, if any don't match, throws before the child ever runs: `delegate("<pkg>", "<agent>") was passed attachmentIds that don't match any attachment in this conversation: "<bad-id>". Copy the id EXACTLY, character for character, from the attachment list — known attachments: <filename> = "<id>", …` — the same "actionable, retryable" shape as `formatDelegateDenial` (`sdk/org/libs/core/src/session/session.ts#Session.resolveDelegateAttachments`).
...
**1. Injection + DTS (typecheck-time).** `policy.mode !== 'none'` becomes the `CapabilityProfile.delegate` flag, so an agent with `canDelegateTo: []` has no `delegate` global *and* no `DELEGATE_DTS` — a stray call fails **typecheck**, not at runtime. This is wired at the session (`sdk/org/libs/core/src/session/session.ts:262`, `:278`), the delegate VM (`sdk/org/libs/core/src/delegate/delegate.ts:170`), and the fork leaf (`sdk/org/libs/core/src/fork/fork.ts:243-245`). The system prompt drops its delegation section too via `omitDelegate: delegatePolicy.mode === 'none'` (`sdk/org/libs/core/src/session/session.ts:268`; `sdk/org/libs/core/src/delegate/delegate.ts:165`).
...
5. **Attachments.** Image `MediaPart`s ride on the child's user message; file attachments contribute id-anchored `readDocument(id)` notes (`sdk/org/libs/core/src/delegate/delegate.ts:259-283`). `delegate/delegate.ts` itself only ever reads the already-resolved `opts.attachments`/`opts.attachmentTexts` — resolving raw `attachmentIds` into those is entirely the caller's job, done once by `resolveDelegateAttachments` and shared by both delegate entry points (see the `attachmentIds` bullet above).
6. **Run the child's turn loop** with a delegate-scoped ForkEngine that inherits the parent's `budgetLimits`, `roleModels`, `forkDepth = depth + 1`, the **shared `dynamicSpaces` map**, and `documentResolver` so a `registerSpace()` inside a fork under the delegate propagates back to the parent and a nested task-fork delegate can still read its uploads. The project session's `codeNodeCtxFactory` is also carried into the child yield router, so a delegated agent's tasklist executes worker-isolated code nodes with the same project-scoped context as the parent (`sdk/org/libs/core/src/delegate/delegate.ts#runDelegate`; `sdk/org/libs/core/src/session/session.ts#Session.runDelegateForFork`; `sdk/org/libs/core/src/session/session.ts#Session.buildYieldContext`). Nested delegate() calls route through `runChildDelegate`, gated by this child's own policy (`sdk/org/libs/core/src/delegate/delegate.ts#runDelegate`).
7. **Result capture.** `currentTask.resolve(value)` records the result synchronously and is always authoritative (`currentTaskResolve` hook, `:247-250`). The action's own tasklist is also tracked for **auto-capture** (`capturableTasklists`, `:188-190`) — but since the research-store-noop fix (`.issues/research-store-noop-diagnosis.md`, cause (a)) the FIRST resolution of a capturable tasklist is stashed as a **fallback only**; it does **not** stop the loop. This lets a "probe, then escalate" pattern complete — e.g. an `answer` tasklist that resolves `{covered:false}`, prompting the model's own next-turn prose plan to run a *differently-named* `research_and_store` tasklist and resolve THAT instead: the model gets to see the probe's result on its next turn before the loop is torn down. A tasklist with the SAME name resolving a **second** time — the model re-running it, most commonly a stuck-loop re-emission that never calls `currentTask.resolve()` at all — IS treated as terminal, capturing the LATEST result (`onTasklistResult`, `:425-441`; `seenCapturableTasklists` tracks which names have already had their fallback). `shouldStop: () => resultCaptured` (`:391`) is what actually ends the loop, checked at the top of the turn loop's NEXT cycle — before a new LLM request — so once either an explicit resolve or a re-emission sets `resultCaptured`, no further turn is requested.
8. **Resolve guarantee.** If still unresolved after the turn loop, up to 2 forced resolve-only nudge turns run on a fresh `Budget({ maxEpisodes: 4 })` (`:471-504`). The final return is `capturedResult` itself (`:518`), not a `resultCaptured ? … : undefined` ternary — `undefined` only when nothing was EVER captured; if the nudges still produce no explicit resolve but a first-pass tasklist fallback (item 7) was already stashed, that fallback is returned instead of being discarded. This mirrors the fork salvage and prevents a specialist that did real work but forgot to resolve from handing `null`/nothing back to the delegator.

> Consent-marked yields (e.g. `installSpace`) **fail closed** inside a delegate: no `requestConsent` is wired for a headless run, so the router's consent gate refuses before install (`sdk/org/libs/core/src/delegate/delegate.ts:412-415`). Store *search/inspect* and manual `emitEvent` still work (system-store itself runs as a delegate of THING).
