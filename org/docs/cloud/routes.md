...
- **`POST /:teamId/members`** (editor) — `{email, role}`, role defaulting to `viewer`. If `zitadel.getUserByEmail` resolves the address they join immediately (`{status:"added"}`); otherwise a pending invite row is written (`{status:"invited"}`) `cloud/gateway/src/routes/teams.ts#teams`. **Either way the address is emailed** `cloud/gateway/src/routes/teams.ts#notifyInvitee` — an invite nobody is told about is not an invite. The two messages differ deliberately: an existing account is told it *was added* and where to open it, while an address with no account is told to sign in *with that address* to claim it, because it otherwise has no idea what is being asked `cloud/gateway/src/lib/team-invite-email.ts#renderTeamInviteEmail`. The link is `TEAM_APP_URL` (default `https://lmthing.team/team`), the teams list that renders a pending invite with its Accept button `sdk/org/apps/web/src/routes/team/index.tsx#TeamsIndex`.

  Sending is **best-effort and reported, never fatal**: the membership or invite row is already committed and is claimable by signing in — not by clicking anything in the mail — so a dead relay must not turn a successful invite into a 500 that invites a retry which re-sends. The response carries `emailed: true|false` so the UI can offer to share the link by hand instead of implying a message arrived. A deployment with no mail transport returns `emailed:false` and still records the invite.

  The team name is free text chosen by an inviter and is interpolated into the message, so it is HTML-escaped through the same helper the sign-in mail uses `cloud/gateway/src/lib/email-login.ts#escapeHtml`.
