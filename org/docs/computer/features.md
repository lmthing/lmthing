...
- `PodTransport` (`Bearer` header at `sdk/org/libs/state/src/lib/pod/transport.ts#PodTransport.headers`, the `401` → `refresh()` → single retry at `transport.ts:69-72`), mounted by `AppProvider` in the `/computer` layout (`sdk/org/apps/web/src/routes/computer/route.tsx#PodReadyTree`) — used by the IDE's file APIs and terminal sockets.
- `useAuth().authFetch` — used by the restart hook (`sdk/org/apps/web/src/routes/computer/use-pod-restart.ts#usePodRestart`) and every settings card.
...
- **There is no "new directory" endpoint.** A folder is created by writing an empty `<dir>/.gitkeep`, which the pod's `handleFsWrite` materializes with `mkdir -p` (`sdk/org/libs/cli/src/server/routes/fs.ts#handleFsWrite`).
...
> **Dead state:** `IdeLayout` renders "Booting…" / "Installing dependencies…" from `store.isBooting` / `store.isInstalling` (`sdk/org/libs/ui/src/computer/ide-layout.tsx:71-74`, `sdk/org/apps/web/src/routes/computer/index.tsx:16-17`). Those flags and their setters exist in the IDE store (`sdk/org/apps/web/src/lib/store.ts#IdeState,56-63`) but **nothing ever sets them** (leftover from the WebContainer era) — in practice only `files.isLoading` drives the spinner.

---

## Dashboard (`/computer/dashboard`)

Renders `<BootProgress tier="flyio">` until the runtime is `running`/`error`, then `<ComputerDashboard>` with status, uptime, metrics, processes, agents, logs and network from `useComputer()` (`sdk/org/apps/web/src/routes/computer/dashboard.tsx#DashboardRoute`). No REST endpoint is involved — everything is meant to arrive over the `/api/ws` control socket (`pod.ts:158-218`).
...
- **The tier is hardcoded `"flyio"`** (`dashboard.tsx:25,34`) even though the actual runtime reports `tier = 'pod'` (`pod.ts:36`); the UI's `RuntimeTier` union is still the legacy `'webcontainer' | 'flyio'` (`sdk/org/libs/ui/src/computer/status-card.tsx#RuntimeTier`).
- **The route is unreachable from the UI.** `ComputerLayout`'s nav labels `/computer` (the IDE) as "Dashboard" and never links `/computer/dashboard` (`computer-layout.tsx:22-27`); `IdeLayout`'s nav omits it entirely (`sdk/org/libs/ui/src/computer/ide-layout.tsx#navItems`). You reach it only by typing the URL.

---

## Terminal (`/computer/terminal`)

A single full-page xterm session (`sdk/org/apps/web/src/routes/computer/terminal.tsx#TerminalRoute`). It calls `createTerminalSession()` from `ComputerContext`, which sends `{type:'terminal.open', sessionId}` over the **control socket** and wraps `terminal.input`/`terminal.resize`/`terminal.close` sends plus `terminal.data` receives into a `TerminalSession` (`pod.ts:93-122`). The pod services those messages by lazily constructing a `TerminalManager` on the control socket (`ensureTerminals`, `ws/agent.ts:17-22`; the message `switch`, `ws/agent.ts:27-46`), rooted at the same `terminalCwd` (`serve.ts:373`).
...
Five cards (`sdk/org/apps/web/src/routes/computer/settings.tsx#Settings`):
...
| `POST /api/compute/ensure`, `GET /api/compute/status`, `GET /api/compute/version`, `POST /api/compute/upgrade` | gateway | `PodEnsureGate` | `sdk/org/apps/web/src/lib/gates.tsx#ensurePod,61,75,99` |
| `GET\|PUT /api/compute/env` | gateway | Env Vars + Models settings | `sdk/org/libs/ui/src/elements/settings/env-vars/index.tsx:30,71` |
| `POST /api/billing/portal` | gateway | Billing settings | `sdk/org/libs/ui/src/elements/settings/billing/index.tsx#openBillingPortal` |
