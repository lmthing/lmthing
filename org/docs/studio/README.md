...
It is not a separate app: `/studio` is a client-side route tree of the same Vite SPA that serves `/chat` and `/computer`; the hostname `lmthing.studio` simply redirects `/` → `/studio` (`sdk/org/apps/web/src/routes/index.tsx:L5-L24`). Unknown hosts (localhost, the `*.test` proxy) also fall back to `/studio` (`sdk/org/apps/web/src/routes/index.tsx#surfaceForHost`).

The same route tree additionally hosts the two **app-distribution** routes — the `/apps` launcher (lmthing.app) and the top-level `/install` installer — because they share Studio's shell (pod gate + pod transport) and hand off to Studio for configuration (`sdk/org/apps/web/src/routes/apps/route.tsx:L24-L67`, `sdk/org/apps/web/src/routes/install.tsx#Route`).
...
No space editor calls a save endpoint. `SpaceProvider` hydrates the whole space file map on mount (`GET /api/projects/:id/spaces/:spaceId/files`) and writes it back as a **1500 ms-debounced whole-space `PUT`** (`sdk/org/libs/state/src/lib/contexts/SpaceContext.tsx#SAVE_DEBOUNCE_MS`, `:L92`, `:L124-L151`). Editors just write into the in-memory VFS (`spaceFS.writeFile` / `deletePath`); the bulk `PUT` is wipe-and-rewrite, so deletions propagate. Non-runnable files (`conversations/`, `.env*`) are stripped before the push (`sdk/org/libs/state/src/lib/pod/transport.ts` `isRunnableSpaceFile`).
...
An unauthenticated arrival (store → install → sign in) survives: `beforeLoad` records the intent in `sessionStorage` (`lmthing_pending_install` / `lmthing_pending_install_space`) and the resume runs both at `/` and in the `/apps` layout, because on lmthing.app the SSO callback lands on the prefixed surface, not the index route (`sdk/org/apps/web/src/routes/install.tsx#Route`; `sdk/org/apps/web/src/routes/apps/route.tsx:L36-L48`).

`/apps` is no longer a launcher — the standalone app surface is gone. The route is a bare redirect into `/chat`, where a project's app loads **inline** on selection (`sdk/org/apps/web/src/routes/apps/index.tsx#Route`, [../chat/README.md](../chat/README.md)); its layout still drops the `access_token` `path=/` cookie so an app's page navigations and relative assets (which cannot send a Bearer header) route to the right pod (`sdk/org/apps/web/src/routes/apps/route.tsx#AppLayout`; `sdk/org/apps/web/src/lib/pod-session.ts` `setPodSessionCookie`), and the `/chat` route does the same for the inline app frame. `APP_PATH_PREFIX` is `''` on lmthing.app and `'/app'` everywhere else (`sdk/org/apps/web/src/lib/config.ts#APP_PATH_PREFIX`). The app itself is documented in [../app/README.md](../app/README.md).
...
|---|---|---|
| Pod cold-wake / status / version / upgrade | gateway `POST /api/compute/ensure`, `GET /api/compute/status`, `GET /api/compute/version`, `POST /api/compute/upgrade` (`sdk/org/apps/web/src/lib/gates.tsx#ensurePod`, `:L61`, `:L75`, `:L99`) | — |
| Pod edge probe + keep-warm | `GET /api/sessions` (`sdk/org/apps/web/src/lib/gates.tsx#waitForPodEdge`), `POST /api/keepalive` every 5 min (`:L202`, `:L339`) | [../cli-api/rest/sessions.md](../cli-api/rest/sessions.md) · [../cli-api/rest/misc.md](../cli-api/rest/misc.md) |
| Projects list / create / delete | `GET`/`POST /api/projects`, `DELETE /api/projects/:id` (`sdk/org/libs/state/src/lib/pod/transport.ts`; sidebar `sdk/org/libs/ui/src/studio/shell/studio-app-sidebar/index.tsx#StudioAppSidebar`) | [../cli-api/rest/projects.md](../cli-api/rest/projects.md) |
