...
| `views:write` | the SPEC writers `writeProjectView`, `writeProjectViewLayout`, `writeProjectViewComponent`, `writeProjectViewShell` — the ONLY UI-authoring surface | bare |
...
The `db:*` grants map to the scoped `db` verbs shown above (`sdk/org/libs/core/src/exec/app-globals.ts:130-170`); `db:schema` additionally earns the live-project writer `writeProjectTable` (`sdk/org/libs/core/src/exec/app-globals.ts:219`). `views:write` earns the four spec writers `writeProjectView`/`writeProjectViewLayout`/`writeProjectViewComponent`/`writeProjectViewShell` (`sdk/org/libs/core/src/exec/app-globals.ts#injectAppGlobals`); `api:write` earns `writeProjectApi`; `hooks:write` earns the live-project `writeProjectHook`/`writeProjectEvent`/`writeProjectFunction` (`sdk/org/libs/core/src/exec/app-globals.ts:213-215`). `project:manage` earns `createProject`/`selectProject` — the LIVE-project create/select globals (`sdk/org/libs/core/src/exec/app-globals.ts:220-223`). The old STORE-CATALOG writers (`writePage`/`writeApi`/`writeHook`/`writeTableSchema`) have been removed.

**`views:write` is the ONLY UI-authoring capability — there is no other id to split it from.** It gates the four spec writers `system-appbuilder` builds its UI with — injected on the grant (`sdk/org/libs/core/src/exec/app-globals.ts#injectAppGlobals`) and declared in the DTS under the same id (`sdk/org/libs/core/src/typecheck/library-dts.ts#PROJECT_VIEW_DTS`) — and the whole guarantee of that builder (a UI that is 100% spec and therefore renders natively with no WebView) is now a fact about the FORMAT rather than a rule an agent is trusted to follow: the model-facing freehand-TSX writers (`writeProjectPage`/`writeProjectComponent`) and the `pages:write` capability id that once gated them were removed from the codebase entirely, so there is no TSX writer left to grant and no second id to keep it out of. A hand-written `.tsx` page can still be SERVED (the store catalog's apps are TSX) but nothing authors a new one.
...
- **Config on a bare-only cap** (`views:write`/`api:write`/`hooks:write`/`project:manage`/`store:read`/`store:install`/`events:emit`, the set `BARE_ONLY_CAPABILITY_IDS`) → "takes no config (bare only)" (`sdk/org/libs/core/src/spaces/capabilities.ts#BARE_ONLY_CAPABILITY_IDS`, `sdk/org/libs/core/src/spaces/capabilities.ts:316-322`).
...
| `system-appbuilder/automator` | `hooks:write`, `db:schema`, `db:read`, `db:write`, `api:write`, `views:write` — `views:write` is the only UI-authoring capability there is, so the UI it authors is necessarily a spec (`sdk/org/libs/core/system-spaces/system-appbuilder/agents/automator/instruct.md:9-15`) |
| `system-appbuilder/spec-builder` | `views:write`, `db:read` — the narrow UI specialist (`sdk/org/libs/core/system-spaces/system-appbuilder/agents/spec-builder/instruct.md:7-9`) |
| `system-appbuilder/data-modeler` · `api-author` | `db:schema`+`db:read` · `api:write`+`db:read` — one slice each (`sdk/org/libs/core/system-spaces/system-appbuilder/agents/data-modeler/instruct.md:7-9`, `sdk/org/libs/core/system-spaces/system-appbuilder/agents/api-author/instruct.md:7-9`) |
...
- [../../project/pages/README.md](../../project/pages/README.md) — the `pages/` UI `views:write` authors
