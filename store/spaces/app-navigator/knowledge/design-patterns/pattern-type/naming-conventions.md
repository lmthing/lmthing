...
Consistent naming prevents errors (broken `flowId` references, missing `enabledKnowledgeFields`) and makes workspaces easier to navigate.
...
**`enabledKnowledgeFields` in agent `instruct.md`:** Must use `"domain-"` prefix + exact folder name

```yaml
enabledKnowledgeFields: ["domain-user-profile", "domain-data-type"]
