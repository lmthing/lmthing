...
**Every app the automator builds ships this dock in `pages/_layout`** — the persistent chrome the router wraps every route in `sdk/org/libs/cli/src/app/runtime/router.tsx:166-170` — so the assistant is on every page by construction, not page-by-page (`sdk/org/libs/core/system-spaces/system-appbuilder/agents/automator/instruct.md:221-250`). A link back to `/chat` is not a dock.
