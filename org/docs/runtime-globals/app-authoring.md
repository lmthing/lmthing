...
The **view-spec** writers: the authoring medium where a page is a validated *object* rather than TSX. They are what `system-appbuilder` — the only shipped app builder — authors with. The TSX writers above still exist and still work, but **no shipped system agent holds `pages:write`**, so nothing authors one today. The spec language, the eight section kinds, the element catalogue and the binding grammar → [view-spec.md](../format/project/pages/view-spec.md).

**`views:write` is a SEPARATE capability from `pages:write`, and that separation is the mechanism, not a taxonomy.** Every `system-appbuilder` agent holds `views:write` and not `pages:write`, so `writeProjectPage` is neither injected nor declared and an attempt to author freehand TSX is a typecheck error the model sees and retries `sdk/org/libs/core/src/exec/app-globals.ts:229-231` · `sdk/org/libs/core/src/typecheck/library-dts.ts#PROJECT_VIEW_DTS`. Sharing one id would not have worked: a `capabilities:` profile lists capability **ids**, not individual globals, so a grant delivers its whole fragment at once `sdk/org/libs/core/src/typecheck/library-dts.ts#CAPABILITY_DTS_FRAGMENTS` · `sdk/org/libs/core/src/exec/bootstrap.ts:378-380`. `buildApp` rides `pages:write` for the same reason, so the builder never has it — its pipeline gates the build host-side, in the `verify` code node, instead `sdk/org/libs/cli/src/app/authoring/globals.ts#ProjectAuthoringGlobals`.
...
**Save-time PARTIAL typecheck — hook-API misuse is caught in the writer, not deferred to `appCheck`.** After the lint, `writeProjectPage` / `writeProjectComponent` / `writeProjectApi` run a single-file `tsc` program over the just-authored source and throw a `LintError` on the first type error `sdk/org/libs/cli/src/app/authoring/save-typecheck.ts#saveTypecheckError` · `sdk/org/libs/cli/src/app/authoring/globals.ts#writeProjectPage`. This is the SAVE tier of a two-tier type check: the NARROWED whole-app typecheck stays at `appCheck` `sdk/org/libs/cli/src/app/build/typecheck.ts#typecheckProjectApp`, but the type-level faults that a verify/fix loop failed to converge on (06-tanzania run 34 shipped a dead app: a handler using `apiHandler` instead of `handler`, `data.items` on an untyped `useApi`, `.mutateAsync`/`.isLoading` on a `useApiMutation` result that has only `{ mutate, isPending, error }`) are each checkable against the ambient alone, so catching them in the writer lets the model fix them in the SAME turn. The save program is built to MIRROR `appCheck` — the same NO-DOM `@app/runtime`+React/JSX ambient `sdk/org/libs/cli/src/app/build/typecheck.ts#buildAmbientDts`, the same per-endpoint NARROWED data hooks whenever the project already has endpoints (the pipeline authors endpoints, node 12, before pages, node 15) `sdk/org/libs/cli/src/app/build/apicall-dts.ts#buildClientApiDts`, and `types/contract.d.ts` / `types/generated.d.ts` loaded as program roots so a bare-global contract type resolves — so a page naming an endpoint that does not exist is a HARD save-time error, not a silent dead section. It differs from `appCheck` in exactly one tolerance, because it is single-file: every unresolved **relative sibling import** (a component authored later in the `forEach`, node 14) and every `node:*` builtin is stubbed as `any` `sdk/org/libs/cli/src/app/authoring/save-typecheck.ts#saveTypecheckFile`, so a not-yet-written sibling never false-rejects and cross-component prop agreement stays an `appCheck` concern. Only a project with **no endpoints at all** degrades to the generic `name: string` hooks (the builder's own zero-endpoint fallback). Added save latency is ~24 ms per file after the one-time (~450 ms) default-lib parse, which is cached module-wide `sdk/org/libs/cli/src/app/authoring/save-typecheck.ts#libFileCache`.
...
`buildApp()`'s three phases prove *compile*-cleanliness, not *runtime*-correctness, so the `verify` gate merges TWO more ground truths beside it and adds one mechanical scan. It lives in a HOST-RUN code node (`sdk/org/libs/core/system-spaces/system-appbuilder/tasklists/build_live_project/16-verify.ts`), not in prompt prose: as prose the model had to re-emit the check on every pass, and in one measured run that was 35% of all build-step errors — a gate that fails to execute reports nothing, which reads as "clean".
...
| `system-appbuilder/automator` | `hooks:write`, `db:schema`, `db:read`, `db:write`, `api:write`, `views:write` | the **live-project** writers `writeProjectTable`/`Api`/`Hook`/`Event`/`Function` plus the three spec writers — **not** `writeProjectPage`/`writeProjectComponent`, which `pages:write` gates and no shipped agent holds `sdk/org/libs/core/system-spaces/system-appbuilder/agents/automator/instruct.md:1-23` |
