...
2. **Launch the replay in the BACKGROUND.** The full run is 30–40 min — longer than the 10-min
   ...
   improvise the stop), and writes `step-NN.json` + appends `trace.md` after EACH step:
       ...
       ART=<ARTIFACT_DIR>   # the dir holding /home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/16/06-tanzania/PROGRESS.md; evidence + your report live here
...
## The report (write it as `report.md` in the same directory as your progress log `/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/16/06-tanzania/PROGRESS.md`)

There is NO per-scenario `results/` dir and NO `run.mjs` — a scenario is just `scenario.yaml` +
`fixtures/`, played by the generic runner. Your report lives beside `/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/16/06-tanzania/PROGRESS.md` in the round's
