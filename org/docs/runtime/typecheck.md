...
**`COMMON_DTS`** — declarations present in **every** VM context (session, fork leaf, delegate): `display`, `inspect`, `loadKnowledge`, `sleep`, the `JSXDescriptor`/`ForkOpts`/`DelegateOpts` interfaces, the classic-JSX `React` and `JSX` namespaces, and host-injected primitives (`readDocument`, `integrationStatus`, `typecheckSource`, `spacePath`, `resolveSpaceDir`, `progress`), plus `catalogDts()` (the design-system catalog as typed JSX globals) appended at the end `sdk/org/libs/core/src/typecheck/library-dts.ts#COMMON_DTS`. Everything in it is injected **unconditionally** by `injectHostTools`, which is what earns it an unconditional declaration; a global whose injection is gated gets its own fragment, gated the same way. `fetch` and `process` are deliberately **not** in this list — see the internal-only-primitives paragraph below.
