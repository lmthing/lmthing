...
The **authoritative** build verdict → `{ ok, built, routes, errors }` from `runProjectAppCheck` — the project-app **typecheck**, then per-endpoint **contract generation**, then the esbuild **bundle**, each short-circuiting on the previous phase's failure `sdk/org/libs/cli/src/server/routes/app-admin.ts#handleAppCheck` · `sdk/org/libs/cli/src/app/build/check.ts#runProjectAppCheck`. Same call as the agent's `buildApp()` global `sdk/org/libs/cli/src/server/session-manager.ts#SessionManager.getProjectAppGlobals` and the `verify` code node, so all three agree. See [runtime-globals/app-authoring.md#buildApp](../../runtime-globals/app-authoring.md) for the three phases and their `phase` values.
