...
The `system-appbuilder` `build_live_project` tasklist uses this as a GATE-AND-RETRY LOOP: after every file is written, the `verify` code node calls the host-side twin `buildProjectApp()`, groups the errors by file, and the host fans out ONE per-file `fix` fork per offending file; `fix` then RESUMES `verify` via `onFail`, so the cycle repeats until clean or the attempt budget is spent. `finalize` runs the ONE authoritative `buildApp()` and resolves `ok` only when the build is clean and complete — never excluding or stubbing a page to make the build pass (see [system-spaces](../system-spaces/README.md)).

`buildApp()`'s three phases prove *compile*-cleanliness, not *runtime*-correctness, so the `verify` gate layers mechanical scans on top of it — each one a check the compiler cannot make. They live in a HOST-RUN code node (`sdk/org/libs/core/system-spaces/system-appbuilder/tasklists/build_live_project/12-verify.ts`), not in prompt prose: as prose the model had to re-emit the scan on every pass, and in one measured run that was 35% of all build-step errors — a gate that fails to execute reports nothing, which reads as "clean".

1. **Endpoint→table** — an api handler's `db.query/insert/update/remove('<table>')` literal is checked against the tables actually on disk (`database/*.json`); a miss builds clean and 500s on every call, because the db surface is dynamically typed `sdk/org/libs/core/system-spaces/system-appbuilder/tasklists/build_live_project/12-verify.ts`.
2. **Page→endpoint** — every `useApi`/`useApiMutation`/`apiCall('<name>')` literal in `pages/`+`components/` is checked against the endpoint `name`s actually exported by `api/`; the client rejects an unknown name **before issuing any request**, so the page renders an error state with nothing in the network panel `sdk/org/libs/core/system-spaces/system-appbuilder/tasklists/build_live_project/12-verify.ts`. This is now ALSO a typecheck error — the ambient types `name` as a literal union of the project's real endpoints `sdk/org/libs/cli/src/app/build/apicall-dts.ts#buildClientApiDts` — so the gate is the second line of defence, not the only one.
3. **Render-correctness** — a page/component function that RETURNS a plain `{ type, props }` object literal — this system's OWN chat/tasklist `display()`-descriptor shape — instead of JSX typechecks (the return type is loose enough) but throws React error #31 ("object with keys {type, props}") at runtime `sdk/org/libs/core/system-spaces/system-appbuilder/tasklists/build_live_project/12-verify.ts`.
4. **Param arity** — a `[id]` route called with no input; the client stringifies the missing value into the path, which still matches and passes validation, returning a plausible 200 carrying the wrong row `sdk/org/libs/core/system-spaces/system-appbuilder/tasklists/build_live_project/12-verify.ts`.
5. **Surface-token-as-text** — `text-<surface token>` paints text in its own background colour; a real Tailwind utility, so it compiles clean `sdk/org/libs/core/system-spaces/system-appbuilder/tasklists/build_live_project/12-verify.ts`.

All of them fold into the SAME `phase: 'gate'` error list, so a file with any of these faults routes to the same per-file `fix` fork, and the `onFail` loop re-runs every scan after each fix round. The matching repair guidance lives in `13-fix.md`; `14-finalize.md` carries any residue rather than re-implementing the scans.
...
| `system-appbuilder/automator` | `hooks:write`, `db:schema`, `db:read`, `db:write`, `pages:write`, `api:write` | the **live-project** writers `writeProject*` `sdk/org/libs/core/system-spaces/system-appbuilder/agents/automator/instruct.md:1-24` |
