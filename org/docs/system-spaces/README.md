...
The **twelve** spaces that ship inside `@lmthing/core` and are loaded into **every** session, fork and delegate. They are what makes an empty project already able to think: THING orchestrates, the architect builds new agents, the appbuilder builds apps (and its frozen sibling the viewbuilder builds the same apps as natively-renderable SPECS), the researcher searches, the engineer codes, and a function toolkit (`remember`/`todoWrite`/…) is in scope everywhere — plus two GRANTED-ONLY functions, `webSearch`/`webFetch`, reachable only via an agent's own `functions:` grant or a tasklist/fork task node's own `functions:` allow-list (§3, §7 of [`runtime-globals/`](../runtime-globals/README.md)); no shipped agent grants them at its own top level today.
...
`SYSTEM_SPACE_NAMES` (`sdk/org/libs/core/src/spaces/system.ts#SYSTEM_SPACE_NAMES`), asserted to be exactly twelve by `sdk/org/libs/core/src/spaces/system.test.ts:L62`:
...
| **`system-appbuilder`** | `automator` | `build_live_project` *(default)* | **THE app builder** — builds/extends the app **in the LIVE project** (the store-catalog `app-architect` cast is gone; the automator authors the whole app itself). Its supplied-material path is a PLAN → per-item BUILD DAG: read all attachments → distil the request + material into the USER STORIES the app must satisfy → make a holistic, BINDING app plan (`plan_app` owns membership; downstream planners only add detail, never add or drop an artifact) → then a `plan → implement` pair per category (tables, endpoints, reusable components, pages), each planner threaded with the stories + the binding plan + the artifacts already built upstream, that the host fans out one file at a time with `forEach`, so a slip on one file no longer loses the whole build (the page list `plan_app` emits stays lightweight — `plan_pages` is itself a per-page `forEach` that details one page per node, so no node holds every page's detail, and a `writeProjectPage` that returns `{ ok:false }` is read and retried, never resolved blind) → a `finalize` node writes the persistent chat layout and reports HONESTLY what landed on disk, flagging any planned page that went missing rather than declaring a partial build a success. Each model-authored implement node carries ✅do/❌never code examples grounded in real generated-code failures (the no-DOM ambient, forbidden imports, endpoint-name drift, raw colors). The implement nodes use the live writers `writeProjectTable` (third `rows` arg seeds data at creation), `writeProjectApi`, `writeProjectComponent`, and `writeProjectPage`; it builds MULTIPLE pages that import the reusable components, and a table-only build is not successful (`sdk/org/libs/core/system-spaces/system-appbuilder/agents/automator/instruct.md:L26-L48`, `:L72-L84`, `tasklists/build_live_project/index.md:L1-L16`). HOST-RUN code-node gates (`validate_contract` before any code exists, then `smoke_endpoints` and `verify` after) mechanically catch the classes of breakage the compiler cannot see — an endpoint querying a table that was never created, a page/component calling `useApi`/`useApiMutation`/`apiCall` with a name that was never generated, and a page/component RETURNING this system's own `{ type, props }` display()-descriptor shape instead of JSX — detailed under `build_live_project` below. This is THING's app path — THING first `createProject`s a target (§4) when the current project is `user`, then delegates the build into it. |
| **`system-viewbuilder`** | `automator` | `build_live_project` *(default)* | **The second app builder — same app, different UI medium.** Same entry point and same input as the appbuilder (`build_live_project`, `{ query, attachmentIds }`), so the two are interchangeable at one call site; tables, API handlers and automations are authored identically (`01-read_sources`, `02-user_stories`, `03-plan_app`, `04-plan_tables`, `07a-plan_automations`, `07b-plan_acceptance`, `10-implement_tables`, `13-smoke_endpoints`, `13a-check_acceptance` and `15a-implement_automations` are copies, and `data-modeler`/`api-author` are the same agents). **Four of the copies have since DIVERGED and are viewbuilder-only**, because a spec page derives its form fields from an endpoint's Input JSON Schema and TSX does not: `05-plan_endpoints` requires an `input` — the request-body keys as `'key: type'` — on any endpoint a `create` section submits to, `09-emit_types` and `11-reconcile_tables` build `Input` from the route `[param]`s PLUS that body (both, because `reconcile_tables` re-emits the whole contract and a params-only twin silently overwrites its own upstream), and `08-validate_contract` rejects such an endpoint with no `input` at plan time — an absent body makes the create page render “Nothing to fill in.” above a Save button (`sdk/org/libs/core/system-spaces/system-viewbuilder/tasklists/build_live_project/08-validate_contract.ts`, `sdk/org/libs/core/system-spaces/system-viewbuilder/tasklists/build_live_project/09-emit_types.ts`). `system-appbuilder`'s own copies are untouched. **What differs is the UI: a page is a SPEC, not TSX** — `plan_view_components`/`plan_views` design it, `implement_view_components`/`implement_views`/`implement_shell` write it with `writeProjectViewComponent`/`writeProjectView`/`writeProjectViewShell`, and every value is bound by path against the ONE endpoint its section names (§6). No agent here holds `pages:write`, so `writeProjectPage`/`writeProjectComponent` are neither injected nor in the DTS and freehand React is a typecheck error rather than a rule (`sdk/org/libs/core/system-spaces/system-viewbuilder/agents/automator/instruct.md:L1-L22`). THING routes here ONLY on an explicit request for a spec-based / natively-rendering app; every other build stays on `system-appbuilder` (§4.1). |
...
flowchart LR
  ...
  THING --> VB["system-viewbuilder/automator<br/>build_live_project — EXPLICIT opt-in only"]
...
**`system-viewbuilder/automator` is on the allowlist but off the default path.** THING delegates to it ONLY when the user explicitly asks for a spec-based / natively-rendering app (or names the viewbuilder); every other build — including both hardcoded `build_live_project` delegates inside `organize_material` and `add_area`, whose task frontmatter pins `canDelegateTo: ['system-appbuilder/automator#build_live_project']` — keeps going to `system-appbuilder` exactly as before (`sdk/org/libs/core/system-spaces/user-thing/agents/thing/instruct.md:L66; sdk/org/libs/core/system-spaces/user-thing/knowledge/playbooks/building/spec-app.md:L5-L39`; `sdk/org/libs/core/src/spaces/system-spaces-dag.test.ts:L149`, `:L199`). The choice is the user's, not a judgement THING makes.
...
| 4a | **Build an app** (always into a LIVE project) | "turn this into an app", "an app for my trip/notes/data", "build me a … app" | If the current project is the default `user` project, THING first asks for a name and `createProject(name)`s a fresh live project — it **never** builds an app into `user`; if the current project is already a real (non-`user`) project, the automator builds in place. Then `delegate('system-appbuilder','automator',{query, attachmentIds})` — the runtime auto-retargets the delegate to the created/selected project — which authors tables (seeding rows), API handlers, reusable components, pages and hooks **directly into the live project**, served at `/app/<project>/` (`:L422-L439`; `sdk/org/libs/core/system-spaces/user-thing/knowledge/playbooks/paths/application.md:L83-L139`, `sdk/org/libs/core/system-spaces/user-thing/knowledge/playbooks/building/create-project.md:L5-L48`) |
...
**Shipped system agents that carry `capabilities:` at the frontmatter level: `system-appbuilder/automator`, all four `system-viewbuilder` agents, `engineer`, `finder`, `thing`; every other system agent parses to `{}`** (`user-memory/memory` earns `db:write` only on its `migrate_to_app_db` write node, not its agent frontmatter). The space-level cap-bearing predicate is asserted by the smoke test `sdk/org/libs/core/src/spaces/capabilities.test.ts:L136-L166` — `system-appbuilder` ∪ `system-viewbuilder` ∪ `integration-*` ∪ `system-engineer` ∪ `system-store` ∪ `user-thing` ∪ `user-memory`.

| Agent | Grants | Unlocks |
|---|---|---|
| `system-appbuilder/automator` | `hooks:write`, `db:schema`, `db:read`, `db:write`, `pages:write`, `api:write` | the LIVE-project writers `writeProjectTable`/`Hook`/`Event`/`Api`/`Page`/`Component` + `db.*` (`system-appbuilder/agents/automator/instruct.md:L10-L16`) |
| `system-viewbuilder/automator` | `hooks:write`, `db:schema`, `db:read`, `db:write`, `api:write`, **`views:write`** | the same data/behaviour writers **minus every TSX writer**, plus the spec writers `writeProjectView`/`writeProjectViewComponent`/`writeProjectViewShell` (`system-viewbuilder/agents/automator/instruct.md:L8-L15`) |
| `system-viewbuilder/spec-builder` | `views:write`, `db:read` | the spec writers only — the narrow UI specialist (`system-viewbuilder/agents/spec-builder/instruct.md:L1-L11`) |
| `system-viewbuilder/data-modeler` · `api-author` | `db:schema`+`db:read` · `api:write`+`db:read` | copies of the appbuilder's specialists, unchanged |
...
> The test's cap-bearing predicate also matches `dir.includes('integration-')` (`sdk/org/libs/core/src/spaces/capabilities.test.ts:L128`). That clause matches **none** of the twelve shipped spaces — it is a leftover from when the `integration-*` spaces (which declare `connections:use`) were bundled; they are store spaces now (`sdk/org/libs/core/src/spaces/system.test.ts:L60-L62`).

---

## 6. The shipped tasklists (the host-driven DAGs)

Seven tasklists ship across four spaces. The tasklist mechanics (`role`, `functions`, `forEach`, `prelude`, `dependsOn`, `goal`, the `{ok, degraded, data}` envelope) are documented in [`../format/space/tasklists/README.md`](../format/space/tasklists/README.md) and [`../runtime/fork-and-tasklists.md`](../runtime/fork-and-tasklists.md) — here is what each shipped DAG actually is.
...
### `system-viewbuilder/build_live_project` — `input: { query, attachmentIds }`

```
read_sources → user_stories → plan_app                    (identical to the appbuilder)
  → plan_tables         → implement_tables       (forEach)
  → plan_endpoints      → implement_endpoints    (forEach)
  → plan_view_components → implement_view_components (forEach: one writeProjectViewComponent object)
  → plan_views          → implement_views        (BOTH forEach: one page's SECTION PLAN, then one
                                                   writeProjectView object literal)
                        → implement_shell        (once: nav + subnav + assistant dock)
  → plan_automations    → implement_automations  (CONDITIONAL, usually zero runs)
  → verify (CODE)       → fix                    (host-run gate → forEach one offending ARTIFACT,
                                                   then onFail RESUMES verify — loops until clean)
  → finalize (goal)                               honest report — runs NO build of its own
```

**The same app, in a different UI medium.** The whole data/behaviour half is a verbatim copy of the appbuilder's (`sdk/org/libs/core/system-spaces/system-viewbuilder/tasklists/build_live_project/01-read_sources.md` … `15a-implement_automations.md`), and `system-appbuilder` is frozen — the copy exists so the two builders can be raced without either changing under the other. What differs is that a page is a **spec**: `{ route, title?, sections: [...] }`, where each section is one of eight kinds and every displayed value is a binding PATH (`$.field`) into the ONE endpoint that section names (`sdk/org/libs/cli/src/app/view-spec/schema.ts#SECTION_KINDS`, `#BINDING_PATTERN`). The element vocabulary a card or row composes from is likewise closed (`sdk/org/libs/cli/src/app/view-spec/schema.ts#ELEMENT_KINDS`). There is no `custom` kind and no per-section code escape, which is what makes the output renderable natively as well as on the web.

**Two rules the endpoint planner carries that the appbuilder's does not**, both forced by that medium (`sdk/org/libs/core/system-spaces/system-viewbuilder/tasklists/build_live_project/05-plan_endpoints.md`):

- **The view-shaped-endpoint rule** — one section, one endpoint, and that endpoint's Output must satisfy every binding the section shows. A cross-table name, a group-by total, a "which one is current" selection, a status label, a percentage and a boolean a row's controls read all become COMPUTED FIELDS on the endpoint, because a spec page has no `.map`, no join and no ternary to compute them with. This is a quality instruction as much as a constraint: of 31 client-side transforms audited across the shipped catalogue, 26 move cleanly this way and usually produce a better result (`design/viewspec-T0-deskcheck.md`).
...
`validate_contract` keeps every one of the appbuilder's checks and adds the view half (`sdk/org/libs/core/system-spaces/system-viewbuilder/tasklists/build_live_project/08-validate_contract.ts`): every section's `kind` is one of the eight, every section's endpoint exists, section ids are unique per page, every `$data.<sectionId>` and every `reveals` target is a section on that SAME page, every `{ use: … }` component reference resolves — and **every `$.`-rooted binding is a declared field of that section's one endpoint**. A miss on the last one is addressed to `plan_endpoints`, not to the page: the endpoint grows the computed field, and the page never grows glue.

`verify` merges THREE ground truths rather than one (`.../16-verify.ts`): the real `ctx.buildProjectApp()` typecheck+bundle over the host-generated page wrappers, `validateAppViews` (the whole-app checks a per-page save cannot make — an orphan route, a dangling nav target, a dead component, a page with no data-bound section) and `renderSmokeViews`, which MOUNTS every view against the app's live endpoint responses over the seeded rows and reports render errors, binding coverage and **empty renders** (both from `sdk/org/libs/cli/src/app/view-spec/validate.ts`). That last gate catches the one failure nothing else can: a page whose every name resolves and whose every binding is contract-valid still shows nothing when the endpoint's computed field is not actually computed. **An always-null binding is therefore routed to the ENDPOINT's file, not the view's** — pointing it at the view would teach `fix` to delete the binding, i.e. to delete the feature.

The appbuilder's TSX-shaped scans are deliberately ABSENT here: a spec has no imports, no JSX and no class names, so the dangling-import, `{ type, props }`-descriptor and surface-token-as-text fault classes cannot be authored. `finalize` runs no build of its own — the last `verify` after the `fix` loop is authoritative — and resolves `ok` only when the build, the app-wide validation AND the render smoke all RAN and were all clean (a gate that did not execute reports nothing, which reads as "clean", so `viewsValidated`/`renderSmoked` are checked explicitly). It also carries forward any `cannotExpress` entry a page's planner recorded: with no escape hatch, an honest "this part needs a multi-select the spec language does not have" is a correct outcome that the user must hear, and `system-appbuilder` remains the home for that class of surface (`.../18-finalize.md`, `.../07-plan_views.md`).
...
| `system-appbuilder/automator` | 781 → 210 lines | The common case answers in ONE statement — `tasklist('build_live_project', …)` — and touches no writer at all, so every line of hook/cron/page-growing detail was charged to a turn that then handed the job to a pipeline whose own step prompts carry that detail |
...
**An aspect nothing names is the failure mode to watch for even WITHOUT a split.** Knowledge is
lazy by default, so a space can ship a correct, expensive aspect that no prompt ever tells its agent
to load. `system-browser`'s `replay-scripts` and `system-viewbuilder`'s `file-formats` /
`capability-model` were exactly that until their instructs named the triples
(`sdk/org/libs/core/system-spaces/system-browser/agents/browser/instruct.md:L45-L57`,
`sdk/org/libs/core/system-spaces/system-viewbuilder/agents/automator/instruct.md:L123-L136`).
