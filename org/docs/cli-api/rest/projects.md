...
**There is no "select project" route.** A project is *selected* by the caller passing `projectId` in the body of `POST /api/sessions` (`sdk/org/libs/cli/src/server/routes/sessions.ts:20-35`), which `createSession` defaults to `DEFAULT_PROJECT_ID` when omitted (`sdk/org/libs/cli/src/server/session-manager.ts:1003`) and resolves to `projectRoot = join(root, projectId)` (`sdk/org/libs/cli/src/server/session-manager.ts:1136`) → [./sessions.md](./sessions.md). The agent-facing `createProject()` / `selectProject()` globals are a *different* thing: they create/select a **live project** under the pod root as the app-build target (`sdk/org/libs/core/src/exec/app-globals.ts#AppGlobalImpls`), backed by the same `createProjectSync` scaffold the REST create path uses (`sdk/org/libs/cli/src/server/projects.ts#createProjectSync`) — see [../../runtime-globals/app-authoring.md](../../runtime-globals/app-authoring.md#createproject--selectproject--picking-the-live-build-target).
