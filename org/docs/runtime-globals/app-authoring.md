...
1. **typecheck** — a real `ts.createProgram` over the project's own `pages/`/`components/`/`api/` sources against a **NO-DOM ambient** (`lib: ['es2020']`, no DOM lib, so `window`/`document`/`navigator`/`alert` are genuinely undeclared and a page must express itself as JSX and React state instead) — plus a small set of host globals that DO exist at runtime and are declared explicitly (`fetch`, `crypto`, `console`, `setTimeout`/`setInterval` and their `clear*` twins), because leaving them undeclared made working code in every shipped store app a permanent error plus a hand-declared `@app/runtime` module and the project's generated row types (`types/generated.d.ts`) `sdk/org/libs/cli/src/app/build/typecheck.ts#typecheckProjectApp`. This is what makes a wrong field name, a prop a component does not declare, a forbidden import, or an undefined identifier a real error instead of code that ships broken.
...
3. **build** — the real esbuild bundle, with a `BuildFailure` captured as structured `{ phase:'build', file, line, message }` errors instead of thrown `sdk/org/libs/cli/src/app/build/pages.ts#buildProjectPagesChecked`.

`ok` ⇔ zero errors across all three phases; `built` ⇔ the esbuild bundle actually emitted (all routes). `errors` carries `{ phase: 'typecheck'|'contract'|'build', file, line?, column?, message }` with `file` project-relative `sdk/org/libs/core/src/db/types.ts#AppCheckError`. The host function is bound to the session's project root `sdk/org/libs/cli/src/server/session-manager.ts#SessionManager.getProjectAppGlobals`; outside a project-app session neither the tasklist twin nor the REST route is reachable.
