...
read_sources (explore)
  ...
  → compile_pass1    → fix_pass1              (buildApp() gate → forEach: fix one offending file)
  → compile_pass2    → fix_pass2              (re-gate → forEach: final per-file fix round)
  → finalize (goal, general)                    _layout + authoritative buildApp() + honest report
```
The automator's default action, and THING's app path. It reads the attachments, distils USER STORIES, then makes one BINDING `plan_app` (which owns membership — the downstream planners only add detail, never add or drop an artifact) before each `plan → implement` category pair. `plan_app` keeps the PAGE LIST lightweight (route + purpose), and `plan_pages` is itself a `forEach` over that list — it details ONE page per node (its endpoints + components), so no single node ever holds every page's detail — before `implement_pages` writes ONE page per node; both page fan-outs mean a slip on one page is salvaged on its own and can never zero the rest. The host fans out `implement_*` **one file at a time**, so a slip on one file no longer loses the whole build; a writer that returns `{ ok:false }` (e.g. a page whose TSX failed to parse) is read and retried, never resolved blind. Then a GATE-AND-RETRY drives DURABLE completeness: `compile_pass1` calls the model-facing `buildApp()` — which runs the write-time lint, a real project-app **typecheck** (NO-DOM ambient; data only through `@app/runtime`) and the esbuild bundle, and returns the STRUCTURED error list (compiler ground truth, not a self-assessment) — groups the errors by file, and the host fans out ONE per-file `fix_pass1` fork per offending file (reading THAT file's real errors + the plan, fixing the specific fault); `compile_pass2`/`fix_pass2` are a second bounded round. Nothing is ever excluded or stubbed to make the build pass — a broken file is FIXED. `finalize` writes the persistent chat layout, runs the ONE authoritative `buildApp()` (the sole build-invoker — it sets `built` for every route), reports HONESTLY what landed on disk (`listProjectDir('pages')`), and resolves `ok` only when the build is CLEAN and complete (`check.ok && check.built && check.errors.length === 0`, nothing missing) — otherwise it FAILS LOUDLY, carrying the residual errors, rather than declaring a partial or type-broken build a success. The implement nodes call the LIVE-project writers `writeProjectTable` (third `rows` arg seeds source-derived data), `writeProjectApi`, `writeProjectComponent`, `writeProjectPage` — and a table-only build is not a completed app (`sdk/org/libs/core/system-spaces/system-appbuilder/tasklists/build_live_project/index.md:L1-L16`, `01-read_sources.md` … `16-finalize.md`).

**`buildApp()` proves compile-cleanliness, not runtime-correctness — the gate nodes layer THREE mechanical, regex-based scans on top of every call to cover the gap**, because each one checks a surface the compiler cannot: the db surface is dynamically typed, and `useApi`'s `name` parameter is a bare `string` in the project-app typecheck ambient (`sdk/org/libs/cli/src/app/build/typecheck.ts:L181-L190`), so neither a wrong table nor a wrong endpoint name is ever a real type error. (1) **Endpoint→table**: an api handler's `db.query/insert/update/remove('<table>')` literal is checked against the tables actually in `database/`; a miss builds clean and 500s on every call, invisible to `buildApp()` (`sdk/org/libs/core/system-spaces/system-appbuilder/tasklists/build_live_project/12-compile_pass1.md:L25-L30`). (2) **Page→endpoint**: every `useApi`/`useApiMutation`/`apiCall('<name>')` literal in `pages/`+`components/` is checked against the endpoint `name`s actually exported by `api/`; a miss compiles clean but the hook silently short-circuits to an error state with NO network request ever firing — live in 06-tanzania run 32 step 3, where `pages/index.tsx` called a `costs-summary` endpoint that was never generated and the homepage showed €0/$0 with nothing in the network tab (`12-compile_pass1.md:L32-L38`). (3) **Render-correctness**: a page/component function that RETURNS a bare `{ type, props }` object literal — this system's OWN chat/tasklist `display()`-descriptor shape, not JSX — typechecks (the return type is loose enough) but throws React error #31 ("object with keys {type, props}") at runtime — live in the SAME run's step 10, where `pages/cash-expenses.tsx`'s `Page()` returned `{ type: 'div', props: {…} }`, resolved `built:true`, and crashed on load (`12-compile_pass1.md:L39-L44`). All three scans fold into the SAME `phase: 'gate'` error list `buildApp()`'s own errors populate, so a caught file is routed to the identical per-file fix fork as a real compiler error, and all three run again in `compile_pass2` (`14-compile_pass2.md:L13-L24`) and in the authoritative `finalize` (`16-finalize.md:L34-L44`) — a first-round fix that re-points a query at the wrong table, invents an endpoint name, or leaves a descriptor return in place is still caught before the app ships. The matching repair guidance — look up the real endpoint in `plan_endpoints.endpoints` and rewrite the call to its `name` VERBATIM; rewrite a `{ type, props }` return as real JSX, a component reference in `type` becoming a JSX tag — lives in `13-fix_pass1.md:L41-L53` and `15-fix_pass2.md:L29-L35`.
