...
`(id uuid, email text, code_hash, link_hash UNIQUE, redirect_uri, attempts int, expires_at, consumed_at, created_at)`, created idempotently by `ensureSchema()` on boot and mirrored in `cloud/migrations/011_email_login_codes.sql` `cloud/gateway/src/lib/db.ts:218-240`. There is deliberately **no `user_id`** column: the mailbox is the key on this path, and the Zitadel user is resolved only after the code verifies. `TTL` is `CODE_TTL_MS` = 15 minutes `cloud/gateway/src/lib/email-login.ts#CODE_TTL_MS`; spent rows older than a day are purged opportunistically `cloud/gateway/src/lib/db.ts#purgeExpiredEmailLoginCodes`.
