...
The `name` + default/`handler` contract is now **enforced at write time**: `writeProjectApi` runs the loader's own name check plus a project-unique-name scan and rejects a violating write with a thrown, retryable error, instead of letting it fail later at load `sdk/org/libs/cli/src/app/authoring/lint.ts#lintApiHandler`. A single-record endpoint named `*-detail` is also rejected when authored as a flat `GET`: it must use a `[param]` segment (for example `recipes/[id]/GET`) so its detail page supplies `$route.id`, rather than mounting an id-less request that schema validation answers with `400` `sdk/org/libs/cli/src/app/authoring/lint.ts#flatDetailRouteError`.
