...
| S15 | migration fan-out: integration spaces + store projects | **DONE**. Batch A: 6 apps, 33 db hooks→event hooks (tests green). Batch B: 9 messaging spaces→events/messages.ts emitter + dropped handler agents (message.received payload; verify translated verbatim). Fixed 3rd bug: '?' optional typeString rejected by validator (core 9145023 + gen-script). sdk/org 9145023, parent 05438db6 | |
| S16 | docs, skills, studio format support (+ consent-card renderer) | **DONE** — events-and-hooks skill + CLAUDE/SPACE_DEVELOPMENT/triggers/webhooks/project-app updates; consent-card renderer wired into chat + studio ask paths (5 tests); @consent function badge; fixed message.posted→received instruct placeholders. sdk/org ab690b2, parent b4542e0a | |
