...
- `TWILIO_AUTH_TOKEN` — the account's Auth Token (secret).
- `TWILIO_FROM_NUMBER` — one of your Twilio numbers in E.164 (the default sender).

On every `callConnection('sms', ...)` the pod:

1. Pins the host to `https://api.twilio.com/2010-04-01/Accounts/<TWILIO_ACCOUNT_SID>` (the SID is
   substituted into the base path).
2. Adds an HTTP **Basic** `Authorization` header built from `TWILIO_ACCOUNT_SID` (username) +
...
If the credentials are missing, `callConnection` throws ("not configured — set TWILIO_AUTH_TOKEN in
...
HMAC-SHA1 of the request URL + sorted POST params, keyed by `TWILIO_AUTH_TOKEN`) before handing the
