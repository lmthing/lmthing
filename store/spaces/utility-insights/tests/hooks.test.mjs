/**
 * Shape guard for utility-insights' hooks — a bad trigger string or budget key ships silently
...
const SPACE_ID = 'utility-insights';
...
test('every hook has a valid shape and stays inside this space', async () => {
  ...
  assert.ok(names.length > 0, 'utility-insights ships at least the weekly digest hook');
...
test('the daily firing triggers the analyst digest action with a bounded budget', async () => {
  const hook = await importHook('weekly-digest');
  assert.equal(hook.type, 'cron');
  assert.equal(hook.daily, '08:00');
  assert.equal(hook.trigger, 'utility-insights/analyst#digest');
  assert.ok(hook.budget?.maxEpisodes > 0 && hook.budget?.maxWallClockMs > 0, 'the cron digest must be budget-bounded');
});
