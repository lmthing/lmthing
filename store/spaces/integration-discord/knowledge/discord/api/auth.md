...
- `INTEGRATION_DISCORD_BOT_TOKEN` — the bot's token (Bot page). Used to authenticate REST calls.
- `INTEGRATION_DISCORD_PUBLIC_KEY` — the application's public key (General Information). Used to verify inbound
  interaction signatures.
- `INTEGRATION_DISCORD_APPLICATION_ID` — the application id (General Information).

The agent never sees, stores, or refreshes these. On every `callConnection('discord', ...)` the pod:

1. Reads `INTEGRATION_DISCORD_BOT_TOKEN` from the pod env (per the space's `connection` descriptor).
...
token is missing, `callConnection` throws ("not configured — set INTEGRATION_DISCORD_BOT_TOKEN …") — ask the user
...
`webhook` descriptor using `INTEGRATION_DISCORD_PUBLIC_KEY` and the `x-signature-ed25519` /
