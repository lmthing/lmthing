...
The fork's user message is assembled from the task `instruction`, a seed-variable summary, an upstream-inputs summary, the JSON output schema, and a final `currentTask.resolve({...})` instruction (`sdk/org/libs/core/src/fork/fork.ts:307-320`). The system prompt carries the role preamble, the parent agent's **charter** (fork-safe identity — `instruct.md` is deliberately NOT injected), the overall tasklist goal, the built-in-globals list (advertising `writeFileRaw`/`execShell` only when the role is write-capable), and full AST-derived signatures of the allowlisted space functions (`sdk/org/libs/core/src/fork/fork.ts:345-421`).
...
Read-only is enforced physically: `roleProfile(role).allowWrite` is false for explore/plan (`sdk/org/libs/core/src/fork/roles.ts:79-82`), which withholds `writeFileRaw`/mutating shell at injection AND drops their DTS declaration (`sdk/org/libs/core/src/exec/bootstrap.ts:324-325`). Read-only roles also lose `registerSpace` and get the `allowWrite`-intersected subset of app capabilities — every mutating grant (`db:write`/`pages:write`/…) is dropped (`sdk/org/libs/core/src/exec/capability.ts:16-28`). Per-role model assignment (`roleModels`) lets cheap explore/plan forks run on a cheaper model (`sdk/org/libs/core/src/fork/roles.ts:60-71`).
