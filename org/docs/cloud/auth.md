...
All gateway JWTs are HS256, signed with a base64-decoded secret from the env var `GATEWAY_JWT_SECRET` `cloud/gateway/src/lib/tokens.ts:3`. `signTokens(userId, email)` issues a pair `cloud/gateway/src/lib/tokens.ts:8-30`:

- **access token** — `sub = userId`, custom claim `{ email }`, `iat` now, `exp` = `ACCESS_TTL` (`"12h"`) `cloud/gateway/src/lib/tokens.ts:5,15-20`. The function also returns `expires_at` computed as `now + 12h` in **seconds** `cloud/gateway/src/lib/tokens.ts:13`.
- **refresh token** — `sub = userId`, custom claim `{ type: "refresh" }`, `exp` = `REFRESH_TTL` (`"30d"`) `cloud/gateway/src/lib/tokens.ts:6,22-27`. Carries no email.

Verification is local via `jose.jwtVerify`:

- `verifyAccessToken(token)` → `{ userId, email }` or `null`; rejects unless `payload.sub` is set and `payload.email` is a string `cloud/gateway/src/lib/tokens.ts:32-42`.
- `verifyRefreshToken(token)` → `{ userId }` or `null`; rejects unless `sub` is set and `type === "refresh"` `cloud/gateway/src/lib/tokens.ts:44-54`.

> Minting a JWT by hand (the workaround for broken password login): sign an HS256 JWT with `sub=<user_id>`, `email` claim, using the base64-decoded `GATEWAY_JWT_SECRET` (from the `lmthing-secrets` k8s secret), then inject it into `localStorage.lmthing_session` — the shape is `AuthSession` `sdk/org/libs/auth/src/types.ts:1-9`.
...
`authMiddleware` `cloud/gateway/src/middleware/auth.ts:16-67` guards every JWT-protected route and sets `c.get("user")` to `{ id, email }` (`AuthUser` `cloud/gateway/src/middleware/auth.ts:9-12`). Order:

1. Require `Authorization: Bearer <token>`; else 401 `cloud/gateway/src/middleware/auth.ts:17-22`.
2. **Local dev only** (`LOCAL_DEV === "true"`): the literal token `"demo"` resolves to `{ id: "local-dev-user", email: "dev@local" }` `cloud/gateway/src/middleware/auth.ts:14,24-28`.
...
4. **Fallback**: Zitadel introspection at `POST {ZITADEL_URL}/oauth/v2/introspect` with HTTP Basic (`ZITADEL_CLIENT_ID:ZITADEL_CLIENT_SECRET`) for any legacy tokens; requires the response `active` plus `sub` and string `email` `cloud/gateway/src/middleware/auth.ts:37-64`.
...
1. `GET /oauth/url?redirect_to=...` (public) builds a success URL `{BASE_URL}/api/auth/oauth/callback?state=<base64url(redirect_to)>` and calls `zitadel.startIdpIntent`, returning `{ url }` — a GitHub OAuth URL `cloud/gateway/src/routes/auth.ts:121-135`. `startIdpIntent` resolves the GitHub IDP id (cached; auto-discovered from `/management/v1/idps/_search`, override with `ZITADEL_GITHUB_IDP_ID`) and `POST`s `/v2/idp_intents`, returning `authUrl` `cloud/gateway/src/lib/zitadel.ts:6,13-35,142-162`.
...
- Session is persisted in `localStorage["lmthing_session"]` as an `AuthSession` `sdk/org/libs/auth/src/client.ts:3,218-227`, `sdk/org/libs/auth/src/types.ts:1-9`.
- `redirectToLogin` sends the browser to `{comUrl}/auth/sso?redirect_uri=&app=&state=`, storing a CSRF `state` in `sessionStorage` `sdk/org/libs/auth/src/client.ts:29-41`.
- `handleAuthCallback` verifies `state`, `POST`s `/api/auth/sso/exchange`, and stores the resulting session `sdk/org/libs/auth/src/client.ts:43-91`.
- `refreshSession` `POST`s `/api/auth/refresh` `sdk/org/libs/auth/src/client.ts:93-116`; `ensureValidToken` refreshes when within `REFRESH_BUFFER` (60s) of `expiresAt`, clearing the session on failure `sdk/org/libs/auth/src/client.ts:7,118-145`.
...
| Var | Used by | Purpose |
|---|---|---|
| `GATEWAY_JWT_SECRET` | tokens.ts | base64-encoded HS256 signing secret `cloud/gateway/src/lib/tokens.ts:3` |
| `ZITADEL_URL` | zitadel.ts, auth middleware | Zitadel instance (`https://auth.lmthing.cloud`) `cloud/gateway/src/lib/zitadel.ts:1` |
| `ZITADEL_SERVICE_PAT` | zitadel.ts | machine-user PAT for the v2 admin API `cloud/gateway/src/lib/zitadel.ts:2,9-11` |
...
| `ZITADEL_GITHUB_IDP_ID` | zitadel.ts | optional; GitHub IDP id, else auto-discovered `cloud/gateway/src/lib/zitadel.ts:6` |
| `BASE_URL` | auth.ts | OAuth callback/failure URL host `cloud/gateway/src/routes/auth.ts:128`, `cloud/gateway/src/lib/zitadel.ts:154` |
