...
2. **Play the scenario from a FRESH, zero-project data dir.** The generic runner wipes the pod
   runtime root, starts a clean server, provisions the one scenario project, plays every
   `scenario.yaml` step, and writes per-step EVIDENCE for you:

       cd sdk/org && SCENARIO_TARGET=local node scenarios/run-yaml.mjs 06-tanzania \
         --fresh-server --out <ARTIFACT_DIR>

   Run it as a BLOCKING FOREGROUND command with a long `timeout` (~25 min): you are a headless
   session, so a backgrounded run is orphaned and killed when your turn ends. `--fresh-server`
   guarantees the "from scratch, zero projects" start on EVERY run and every verify-rerun.
   `<ARTIFACT_DIR>` = the directory that holds `/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/2/06-tanzania/PROGRESS.md`, so the evidence is captured with
   the round's artifacts (and never in a scenario `results/` dir — there isn't one).
3. **Read the evidence — this is your source of truth, never prose:** `<ARTIFACT_DIR>/trace.md`
   (human-readable, each step with its `expect` checklist) and `<ARTIFACT_DIR>/step-NN.json`
   (structured: the turn's delegates / yields / errors / tokens / reply, the asks, and the real
   spaces + app tables + DB rows after the step).
4. **The verify-rerun** (after a fix): rebuild if you touched code, then replay from scratch THROUGH
   the previously-failed step —

       cd sdk/org && SCENARIO_TARGET=local node scenarios/run-yaml.mjs 06-tanzania \
         --fresh-server --through <N> --out <ARTIFACT_DIR>

   A system-space prompt/tasklist fix needs the rebuild too (the fresh server adopts the rebuilt
   `dist/system-spaces` tree on boot). `--plan` dry-prints the step list without touching a pod.
...
## The report (write it as `report.md` in the same directory as your progress log `/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/2/06-tanzania/PROGRESS.md`)

There is NO per-scenario `results/` dir and NO `run.mjs` — a scenario is just `scenario.yaml` +
`fixtures/`, played by the generic runner. Your report lives beside `/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/2/06-tanzania/PROGRESS.md` in the round's
