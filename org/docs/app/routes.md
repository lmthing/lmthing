...
| `* /:projectId/api/*` | the SAME `appApiHandler` | only when `LMTHING_GATEWAY_URL` is set `sdk/org/libs/cli/src/server/serve.ts:322-324` |
| `* /:projectId/*` | `createPageServeHandler(getOutDirForProject, '')` (mountPrefix `''`) | only when `LMTHING_GATEWAY_URL` is set `sdk/org/libs/cli/src/server/serve.ts:322-325` |
...
The same patterns are matched **client-side** at runtime: `matchRoutes` splits pattern and path into segments and captures `:param`s `sdk/org/libs/cli/src/app/runtime/router.tsx:57-75`; the matched page renders inside `_layout` inside `_app` `sdk/org/libs/cli/src/app/runtime/router.tsx:167-205`; an unmatched path renders a minimal `No page for <path>` `sdk/org/libs/cli/src/app/runtime/router.tsx:207-214`. There is no pod-side page routing beyond the SPA fallback below.
