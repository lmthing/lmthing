...
| | `automator` | *(none — model-driven)* | Builds/extends the app **in the LIVE project**: `writeProjectTable` (with a third `rows` arg that seeds data at creation), `writeProjectHook`, `writeProjectEvent`, `writeProjectApi`, `writeProjectPage`, and `writeProjectComponent` for reusable in-app UI (`sdk/org/libs/core/system-spaces/system-appbuilder/agents/automator/instruct.md:L16-L40`). This is THING's default app path. |
