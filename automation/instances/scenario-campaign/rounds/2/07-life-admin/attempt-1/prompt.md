# Scenario campaign — `07-life-admin` · round 2 (ADD ACTS (batch 2))

`07-life-admin` already has a runner and has been extended in earlier rounds. This round: **add
ANOTHER batch of additional Acts and continue from there.** Never regress the existing Acts —
extension is strictly additive.

1. **Orient & resume.** Read `sdk/org/scenarios/07-life-admin/scenario.md` (esp. its Acts table + Actual-results), `sdk/org/scenarios/07-life-admin/run.mjs`,
   and `sdk/org/scenarios/07-life-admin/results/` (report, trace, checkpoint) to see exactly which Acts exist, which passed,
   and any open frontier a prior round recorded. Read this run's `/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/2/07-life-admin/PROGRESS.md` if it exists.
2. **Pick the next batch of Acts (2–4)** from the feature catalog below — capabilities
   `07-life-admin` still does **not** cover — extending the same persona / same growing project
...
3. **Add them to `sdk/org/scenarios/07-life-admin/scenario.md` (Acts table + user stories/expectations) AND `sdk/org/scenarios/07-life-admin/run.mjs` (1:1)**,
...
- **Scenario:** `07-life-admin` (`sdk/org/scenarios/07-life-admin`)  ·  **Round:** 2 — ADD ACTS (batch 2)  ·  **Branch:** `main`
...
(there is no CI/deploy in this loop — see **Target** below). Scenario files live at `sdk/org/scenarios/07-life-admin`;
...
- **Set `SCENARIO_TARGET=local`** in the environment of every harness command (`provision.mjs`,
  `sdk/org/scenarios/07-life-admin/run.mjs`, any probe). With it set, `getUser` returns `{ pod:'http://localhost:8080', token:null }`
...
Extend `07-life-admin` with **additional Acts**, run the **whole scenario end-to-end against the
...
1. **catalog-scout** — READ-ONLY prep: read sdk/org/scenarios/07-life-admin/scenario.md + sdk/org/scenarios/07-life-admin/run.mjs + sdk/org/scenarios/07-life-admin/results and the feature catalog in this prompt; list which catalog capabilities 07-life-admin does NOT yet cover, and propose the next 2–4 Acts (name + what each asserts on the trace/real state). Do NOT provision, open a THING session, or send any turn — analysis only.
...
testing a script. Hold them in `sdk/org/scenarios/07-life-admin/scenario.md` and in every message `sdk/org/scenarios/07-life-admin/run.mjs` sends.
...
A scenario is TWO files plus generated results, all under `sdk/org/scenarios/07-life-admin`:
...
- **Every fixture must carry at least one fact/token that appears in NO other fixture**, and an Act
  ...
  assertion worthless.
- Fixtures are **committed and self-contained** under `sdk/org/scenarios/07-life-admin/fixtures/`. Record where each
  came from (source URL) in `fixtures/links.md`.
- **Wire it into BOTH** `scenario.md` (the user flow + the Acts table) and `sdk/org/scenarios/07-life-admin/run.mjs` (uploaded,
...
Pick additional Acts that exercise catalog capabilities `07-life-admin` does **not** yet cover.
...
**Never exercised — pick 2–4 that fit `07-life-admin`'s persona and build them into the story.** A
...
`07-life-admin` — recovered errors stay a metric, unrecovered ones fail the run.
...
const user  = await getUser('07-life-admin');            // { userId, token, pod }
...
const r = new Report('07-life-admin', 'title');
r.step('Act VI — …', 'expected'); r.check('label', pass, actual); r.metric('turn', s, 's');
r.save('sdk/org/scenarios/07-life-admin/results/report.md'); r.saveTrace('sdk/org/scenarios/07-life-admin/results/trace.json', thing);
```
...
`sdk/org/scenarios/07-life-admin/run.mjs`, and if the product cannot do them yet, **implement the missing feature** (§4 fix loop:
...
1. **Bring up the local pod + smoke.** `cd sdk/org/scenarios/harness && SCENARIO_TARGET=local node
   ...
   `SCENARIO_TARGET=local node provision.mjs 07-life-admin` (returns `{ pod:'http://localhost:8080',
...
2. **Run Act by Act, in the FOREGROUND.** You are a **headless `claude -p` session**: when you stop
   ...
   until it exits, e.g. `cd sdk/org/scenarios/harness && node sdk/org/scenarios/07-life-admin/run.mjs --acts=1` with the Bash
...
Maintain the per-run progress log at **/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/2/07-life-admin/PROGRESS.md**.
...
- `sdk/org/scenarios/07-life-admin/scenario.md` has all six sections + the feature checklist + the Acts table. **Round 1: do not
  rewrite the story** — it was authored deliberately; implement it. Later rounds: add the new Acts.
- `sdk/org/scenarios/07-life-admin/run.mjs` **exists and implements `scenario.md` Act for Act (1:1)** — on round 1 you are writing
...
- **From round 2 on: at least one NEW real fixture** sourced from the real web (or Azure-TTS-generated
  for audio), committed under `fixtures/`, uploaded by `sdk/org/scenarios/07-life-admin/run.mjs`, and asserted via a token that
...
- `sdk/org/scenarios/07-life-admin/results/` has the report + trace; the `.md` **Actual results** section is filled with:
