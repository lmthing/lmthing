...
| `enum` | Optional CLOSED value domain for a `string` column — the exhaustive set of values it may hold (`["paid","owed","unconfirmed"]`) `sdk/org/libs/core/src/db/schema.ts#ColumnSchema`. Advisory to storage (not a SQL `CHECK`), but the appbuilder's `emit_types` renders it as a string-literal UNION on the generated row type, so a handler comparing the column against a value the domain never had is a compile error `sdk/org/libs/core/system-spaces/system-appbuilder/tasklists/build_live_project/09-emit_types.ts#columnTsType`. |
