...
> contradict the shipped tiers (`cloud/gateway/src/lib/tiers.ts`): the real gateway markup is 15%
> (`cloud/scripts/generate-litellm-models.ts`), and the runtime has **no "Flows" concept** (it has
> tasklists — see `org/docs/format/space/`). The pod/K8s lifecycle below is not owned by this
> directory: [`org/docs/devops/infrastructure.md`](../org/docs/devops/infrastructure.md) is
> authoritative. Preserved to keep the thinking, not to bless it.
