/**
 * Shape guard for utility-validator's hooks — a bad trigger string or budget key ships silently
...
const SPACE_ID = 'utility-validator';
...
test('every hook has a valid shape and stays inside this space', async () => {
  ...
  assert.ok(names.length > 0, 'utility-validator ships at least the daily check hook');
...
test('the daily check triggers the inspector check action with a bounded budget', async () => {
  const hook = await importHook('daily-check');
  assert.equal(hook.type, 'cron');
  assert.equal(hook.daily, '06:30');
  assert.equal(hook.trigger, 'utility-validator/inspector#check');
  assert.ok(!/#bind$/.test(hook.trigger), 'binding proposes rules for a human — it is not a cron job');
  assert.ok(hook.budget?.maxEpisodes > 0 && hook.budget?.maxWallClockMs > 0, 'the cron check must be budget-bounded');
});
