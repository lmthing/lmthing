...
| `user-<id>` | One per user (all tiers, lazily) — a `lmthing` Deployment (the compute pod), its `Service`, a `user-data` PVC, and `acr-pull-secret` / `user-env` Secrets | The gateway, via the K8s API (`cloud/gateway/src/lib/compute.ts:172-357`) |
