...
- `pages` — `{ routePath, file }`, discovered by a **read-only** walk that mirrors the page build (so the manifest never triggers a rebuild) `app-admin.ts#discoverPageRoutes`. A **spec (viewbuilder) app** has no `pages/` dir — its pages are `views/*.view.json`, and the generated wrappers were dropped — so its routes are added from `views/` through the **same** `viewRoutePath` served-route mapping `sdk/org/libs/cli/src/server/routes/app-admin.ts#discoverSpecRoutes`; without this the chat sidebar's `APP` section is empty for exactly the apps the native renderer can draw. A **migrated** app can carry BOTH — a generated `pages/` wrapper and the `views/` spec for the same route — so the two lists are **deduped by `routePath`** (the spec entry wins, the wrapper being a leftover); otherwise every page is listed twice `sdk/org/libs/cli/src/server/routes/app-admin.ts#handleAppManifest`. `hasViews` (`sdk/org/libs/cli/src/app/loader.ts#loadProjectApp`) is what makes a pure views-only project report `hasApp:true`.
...
The **authoritative** build verdict → `{ ok, built, routes, errors }` from `runProjectAppCheck` — the project-app **typecheck**, then per-endpoint **contract generation**, then the esbuild **bundle**, each short-circuiting on the previous phase's failure `sdk/org/libs/cli/src/server/routes/app-admin.ts#handleAppCheck` · `sdk/org/libs/cli/src/app/build/check.ts#runProjectAppCheck`. Same call as the tasklist `verify` code node's `ctx.buildProjectApp()` (`sdk/org/libs/cli/src/app/authoring/globals.ts#ProjectAuthoringGlobals`), so both agree. See [runtime-globals/app-authoring.md](../../runtime-globals/app-authoring.md) for the three phases and their `phase` values.
