...
for (const size of LLM_MODELS) {
  describe(`space-computer: ComputerAdmin [${size}]`, () => {
    it.skipIf(!hasKeys)('explains compute pod management', { timeout: 90_000 }, async () => {
      const { Session, AgentLoop, resolveModel } = await import('lmthing')

      const instruct = readFileSync(join(SPACE, 'agents/agent-computer-admin/instruct.md'), 'utf-8')
      const session = new Session()
      const model = resolveModel(size)
      const loop = new AgentLoop({ session, model, modelId: size, instruct, maxTurns: 3 })

      await loop.handleMessage('How do I set an environment variable on my compute pod so my agent can use a custom API key?')
  ...
  describe(`space-computer: Troubleshooter [${size}]`, () => {
    it.skipIf(!hasKeys)('guides through troubleshooting a crashed pod', { timeout: 90_000 }, async () => {
      const { Session, AgentLoop, resolveModel } = await import('lmthing')

      const instruct = readFileSync(join(SPACE, 'agents/agent-troubleshooter/instruct.md'), 'utf-8')
      const session = new Session()
      const model = resolveModel(size)
      const loop = new AgentLoop({ session, model, modelId: size, instruct, maxTurns: 3 })

      await loop.handleMessage('My compute pod shows status "CrashLoopBackOff". What should I check first?')
