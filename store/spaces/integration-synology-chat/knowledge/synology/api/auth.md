...
- `INTEGRATION_SYNOLOGY_CHAT_BASE_URL` — the NAS origin, e.g. `https://nas.example.com:5001` (the part of the
  incoming-webhook URL before `/webapi/...`).
- `INTEGRATION_SYNOLOGY_CHAT_TOKEN` — the **incoming** webhook token (the `token=` value in the incoming-webhook
  URL). Used to authenticate outbound sends.
- `INTEGRATION_SYNOLOGY_CHAT_OUTGOING_TOKEN` — the **outgoing** webhook token, used pod-side to verify that inbound
  events genuinely came from the user's NAS.

On every `callConnection('synology', ...)` the pod:

1. Host-pins to `INTEGRATION_SYNOLOGY_CHAT_BASE_URL` and forwards the relative `path` + `query`.
2. Appends `?token=<INTEGRATION_SYNOLOGY_CHAT_TOKEN>` (the declared `query-token` auth style) to authenticate.
...
Inbound (outgoing-webhook) events are verified pod-side using `INTEGRATION_SYNOLOGY_CHAT_OUTGOING_TOKEN`: the pod
...
INTEGRATION_SYNOLOGY_CHAT_TOKEN …"). In that case, ask the user to fill in their Synology Chat details in **the
...
certificate actually covers in `INTEGRATION_SYNOLOGY_CHAT_BASE_URL`.
