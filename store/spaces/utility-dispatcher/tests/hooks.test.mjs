/**
 * Shape guard for utility-dispatcher' hooks — a bad trigger string or budget key ships silently
...
const SPACE_ID = 'utility-dispatcher';
...
test('every hook has a valid shape and stays inside this space', async () => {
  ...
  assert.ok(names.length > 0, 'utility-dispatcher ships at least the daily dispatch hook');
...
test('the daily dispatch triggers the dispatcher dispatch action with a bounded budget', async () => {
  const hook = await importHook('daily-dispatch');
  assert.equal(hook.type, 'cron');
  assert.equal(hook.trigger, 'utility-dispatcher/dispatcher#dispatch');
  assert.ok(hook.budget?.maxEpisodes > 0 && hook.budget?.maxWallClockMs > 0, 'the cron dispatch must be budget-bounded');
});
