...
The recognized type strings are `string`, `number`, `boolean`, `object`, `array`, and `any`; a trailing `?` marks the field optional (an absent or `undefined` value passes, a present value is checked against the base type), and unknown base types are treated leniently (`sdk/org/libs/core/src/tasklist/schema.ts#checkField`). This is the same type-string vocabulary used by step `output`/`input` validation (`sdk/org/libs/core/src/tasklist/schema.ts#validateOutput`).
