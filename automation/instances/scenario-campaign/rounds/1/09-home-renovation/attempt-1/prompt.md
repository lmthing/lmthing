...
(there is no CI/deploy in this loop — see **Target** below). Scenario files live at `sdk/org/scenarios/09-home-renovation`;
...
- **Set `SCENARIO_TARGET=local`** in the environment of every harness command (`provision.mjs`,
  `sdk/org/scenarios/09-home-renovation/run.mjs`, any probe). With it set, `getUser` returns `{ pod:'http://localhost:8080', token:null }`
...
Extend `09-home-renovation` with **additional Acts**, run the **whole scenario end-to-end against the
...
testing a script. Hold them in `sdk/org/scenarios/09-home-renovation/scenario.md` and in every message `sdk/org/scenarios/09-home-renovation/run.mjs` sends.
...
- **Every fixture must carry at least one fact/token that appears in NO other fixture**, and an Act
  ...
  assertion worthless.
- Fixtures are **committed and self-contained** under `sdk/org/scenarios/09-home-renovation/fixtures/`. Record where each
  came from (source URL) in `fixtures/links.md`.
- **Wire it into BOTH** `scenario.md` (the user flow + the Acts table) and `sdk/org/scenarios/09-home-renovation/run.mjs` (uploaded,
...
**Never exercised — pick 2–4 that fit `09-home-renovation`'s persona and build them into the story.** A
...
`09-home-renovation` — recovered errors stay a metric, unrecovered ones fail the run.
...
`sdk/org/scenarios/09-home-renovation/run.mjs`, and if the product cannot do them yet, **implement the missing feature** (§4 fix loop:
...
1. **Bring up the local pod + smoke.** `cd sdk/org/scenarios/harness && SCENARIO_TARGET=local node
   ...
   `SCENARIO_TARGET=local node provision.mjs 09-home-renovation` (returns `{ pod:'http://localhost:8080',
...
- `sdk/org/scenarios/09-home-renovation/scenario.md` has all six sections + the feature checklist + the Acts table. **Round 1: do not
  rewrite the story** — it was authored deliberately; implement it. Later rounds: add the new Acts.
- `sdk/org/scenarios/09-home-renovation/run.mjs` **exists and implements `scenario.md` Act for Act (1:1)** — on round 1 you are writing
...
- **From round 2 on: at least one NEW real fixture** sourced from the real web (or Azure-TTS-generated
  for audio), committed under `fixtures/`, uploaded by `sdk/org/scenarios/09-home-renovation/run.mjs`, and asserted via a token that
