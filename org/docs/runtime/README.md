...
- **The dynamic plan (soft reminder).** The plan is the checklist the agent maintains with the `todoWrite` system function, persisted to `<spaceDir>/.lmthing/todos.json` (statuses `pending`/`in_progress`/`completed`/`failed`). A `beforeTurn` reminder provider keeps the agent on it, transient (appended to a single request, never persisted): `openPlanReminder` re-injects every not-`completed` task on **every** top-level request so the agent keeps the statuses current, and re-surfaces `failed` tasks (marked `✗`) so the agent must acknowledge them rather than silently drop them (`sdk/org/libs/core/src/session/plan-reminders.ts#openPlanReminder`; registered alongside the session-naming nudge in `sdk/org/libs/core/src/session/session.ts#Session` and consumed at `sdk/org/libs/core/src/eval/turn-loop.ts:585-588`). Forks/delegates do not set `beforeTurn`.
