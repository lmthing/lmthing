...
There is also **no `writeComponent` authoring global** — the authoring surface exposes `writePage`/`writeApi`/`writeHook`/`writeTableSchema` (catalog) and their live-project twins `writeProjectPage`/`writeProjectApi`/… (`sdk/org/libs/cli/src/app/authoring/globals.ts:231-233,433`, `AppGlobalImpls` at `:25-27`), but nothing that targets `components/`. Components are ordinary source files the app-builder writes directly, then imported by the generated pages.
