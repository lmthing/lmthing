...
**Conversation history is a switcher INSIDE the dock, not a sidebar.** The `<Chat>` panel's own chrome carries a history control that lists the project's past sessions (`GET /api/projects/:id/sessions`) and resumes or starts one in place (`sdk/org/libs/cli/src/app/runtime/chat.tsx:146-164`, `sdk/org/libs/cli/src/app/runtime/chat-protocol.ts#listChatSessions`) — so once a project has grown into an app, "open the chat" is where you find and switch conversations. The `chat` **spec section** additionally accepts `suggestions?: string[]` (`sdk/org/libs/cli/src/app/view-spec/schema.ts#ChatSection`), first-run prompt chips the shared `ReplChatView` renders above the input while the transcript is still empty (`sdk/org/libs/ui/src/chat/components/ReplChatView.tsx#ReplChatView`); the newborn chat index seeds them so a blank project is never a blank slate.
