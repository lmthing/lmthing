...
2. On a read miss — and only when the path does **not** already end in `.md` — fall back to `<path>.md` (a direct option/overview file, returned verbatim), then `<path>/index.md` `sdk/org/libs/core/src/globals/load-knowledge.ts#readWithKnowledgeFallback`. This is why the prompt can hand the model an extension-less aspect **slug** and `loadKnowledge('domain','field','aspect')` still resolves. A bare `loadKnowledge('domain','field')` resolves to the field's `index.md` — the **menu** — and is augmented with the field's **real option list read from disk** (every `<slug>.md` except `index.md`, sorted) appended in full, so the model always sees the exact option names it can pass as a third argument instead of guessing one the hand-written menu forgot to list `sdk/org/libs/core/src/globals/load-knowledge.ts#listKnowledgeOptions`.
3. Parse:
