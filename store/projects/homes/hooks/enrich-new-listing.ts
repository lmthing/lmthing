export default {
  type: 'event',
  on: { event: 'project/db.listings.insert' },
  ...
  handler: async ({ input, db, delegate }: { input: any; db: any; delegate: (ref: string, action: string, opts: { input: unknown }) => Promise<{ ok: boolean; result?: unknown; error?: string; sessionId?: string }> }) => {
    ...
    const existing = await db.query('listing_analyses', { where: { listingId: input.id } });
    ...
    // never back to `listings` in a way that would re-fire THIS hook's `insert` event
    ...
    await delegate('intake/surveyor', 'normalize', { input: { listingId: input.id } });
    await delegate('scout/analyst', 'analyze', { input: { listingId: input.id } });
    await delegate('scout/locator', 'locate', { input: { listingId: input.id } });
    await delegate('scout/ranker', 'rank', { input: { listingId: input.id } });
  },
};
