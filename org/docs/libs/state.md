...
All file data lives in one flat `Map<string, string>` inside `AppFS` (`sdk/org/libs/state/src/lib/fs/AppFS.ts#AppFS`). A "path" is a `/`-joined key; there are no directory objects — directories are inferred from key prefixes at read time (`readDir`, `sdk/org/libs/state/src/lib/fs/AppFS.ts#AppFS.readDir`). `FileTree` is the exported alias `Record<string, string>` (`sdk/org/libs/state/src/types/project.ts#FileTree`).
...
These are exactly the keys the `P` path-builder object emits (`sdk/org/libs/state/src/lib/fs/paths.ts#P`); `P.globs.*` supplies the matching glob patterns (e.g. `allAgents: 'agents/*/instruct.md'`, `allKnowledgeIndexes: 'knowledge/*/*/index.md'`, `tasklistTasks(name)`; `sdk/org/libs/state/src/lib/fs/paths.ts:71-107`).

### `AppFS` public API (`FSInterface`)

`AppFS` and every scoped class implement `FSInterface` (`sdk/org/libs/state/src/lib/fs/FSInterface.ts#FSInterface`):
...
- `writeFile` emits `create` if the key is new, else `update` (`sdk/org/libs/state/src/lib/fs/AppFS.ts#AppFS.writeFile`). `deletePath`/`renamePath`/`duplicatePath` operate on a **prefix** (the key itself plus everything under `prefix + '/'`), emitting one event per affected key (`sdk/org/libs/state/src/lib/fs/AppFS.ts:103-164`).
- `batch(ops)` wraps a `FileOp[]` (`write`/`append`/`delete`/`rename`/`duplicate`; `sdk/org/libs/state/src/types/project.ts#FileOp`) in `bus.beginBatch()/endBatch()` so subscribers get one coalesced `BatchEvent` (`sdk/org/libs/state/src/lib/fs/AppFS.ts#AppFS.batch`).
- `import(data)` clears the store, re-seeds it, then re-emits a `create` per key (`sdk/org/libs/state/src/lib/fs/AppFS.ts#AppFS.import`).

### Scoped views: `ProjectFS` / `SpaceFS`

`ScopedFS` wraps a root `AppFS` with a path prefix and transparently joins the prefix on the way in and strips it on the way out — for reads, writes, glob results, **and events** (`sdk/org/libs/state/src/lib/fs/ScopedFS.ts#ScopedFS`). `joinPath`/`stripPrefix`/`normalizePrefix` (`ScopedFS.ts:8-28`) handle the prefix arithmetic. So a `SpaceFS` consumer only ever sees space-relative paths like `agents/bot/instruct.md`.

- `ProjectFS` scopes to `<projectId>` (`sdk/org/libs/state/src/lib/fs/ScopedFS.ts#ProjectFS`).
- `SpaceFS` scopes to `<projectId>/<spaceId>` (`sdk/org/libs/state/src/lib/fs/ScopedFS.ts#SpaceFS`); `SpaceFS.fromProjectFS(projectFS, spaceId)` derives it from a `ProjectFS` (`ScopedFS.ts:315-321`).
...
`AppFS` owns one `FSEventBus` (`sdk/org/libs/state/src/lib/fs/AppFS.ts#AppFS`). Every mutation calls `bus.emit(event)`; when a batch is open the event is queued instead (`sdk/org/libs/state/src/lib/fs/FSEventBus.ts#FSEventBus.emit`). `dispatchEvent` fans a single `FSEvent` (`{type, path, oldPath?, content?, timestamp}`; `sdk/org/libs/state/src/lib/fs/events.ts:3-11`) to the listener families below, in this order (`FSEventBus.ts:71-141`):
...
- **`DraftStore`** — an in-memory `Map` of unsaved edits, entirely separate from `AppFS` (`sdk/org/libs/state/src/lib/fs/DraftStore.ts#DraftStore`). `set/get/has/delete/clear`, bulk `getAll/getPaths/isEmpty/getCount`, per-path `onChange(path, cb)`, and `useSyncExternalStore`-compatible `subscribe`/`getSnapshot`. Drafts never persist and never touch the pod — the editor writes a draft on keystroke, then commits it to the FS and deletes the draft on save.
- **`UIStore`** — ephemeral transient view state (expanded sections, open modals) as a `Map<string, unknown>` with a memoized snapshot; `get/set/delete/has/clear` + `subscribe`/`getSnapshot`, `set` no-ops when the value is `Object.is`-equal (`sdk/org/libs/state/src/lib/fs/UIStore.ts#UIStore`).
...
**FS-instance hooks** (`sdk/org/libs/state/src/hooks/fs/index.ts:3-12`): `useAppFS()` → the root `AppFS` (`useAppFS.ts:7-10`); `useProjectFS()` → `ProjectFS | null` (`useProjectFS.ts:6-9`); `useSpaceFS(spaceId?)` → `SpaceFS | null`, defaulting to the context space (else deriving one from `currentSpaceId`, or from an explicit `spaceId`) (`sdk/org/libs/state/src/hooks/fs/useSpaceFS.ts#useSpaceFS`).

**File/dir/glob hooks — all of them read through `useSpaceFS()`, i.e. they are SPACE-scoped** (see the gotcha below): `useFile(path)` subscribes to that one file and returns its content or `null` (`sdk/org/libs/state/src/hooks/fs/useFile.ts#useFile`); `useFileWatch` (`useFile.ts:16-22`); `useDir`/`useDirWatch` (`useDir.ts:10-48`); `useGlob`/`useGlobWatch` (`useGlob.ts:10-58`); `useGlobRead` → `FileTree` (`useGlobRead.ts:10-33`); `useFileFrontmatter<T>` (`useFileFrontmatter.ts:10-24`); `useFileConfig<T>` (`useFileConfig.ts:9-25`); `useStreamWrite`/`useStreamAppend` (`useStreamWrite.ts:6-46`).
...
**UI-state hooks:** `useUIState<T>(key, initial)` and `useToggle(key, initial)` over `UIStore` (`sdk/org/libs/state/src/hooks/useUIState.ts:16-54`); `useAsyncAction<T>(key)` → `{isLoading, error, data, execute, reset}`, its state parked in `UIStore` under `key` (`sdk/org/libs/state/src/hooks/useAsyncAction.ts#useAsyncAction`).

**Typed-format hooks** (parser + FS composed) — the full catalog, all re-exported from `hooks/index.ts:3-22`. Every one of them is a `useFile`/`useDir`/`useGlob(Read)` read piped through the matching parser inside a `useMemo`, and every one **swallows parse errors and returns `null`** rather than throwing (`sdk/org/libs/state/src/hooks/agent/useAgentInstruct.ts#useAgentInstruct`, `hooks/tasklist/useTasklistIndex.ts:29-36`, `hooks/knowledge/useKnowledgeFieldIndex.ts:15-22`, `hooks/workspace/usePackageJson.ts:19-26`):
...
- **Every file/dir/glob hook is SPACE-scoped, including the ones named "project".** `useFile`/`useGlob`/`useDir`/`useGlobRead`/`useFileConfig`/`useFileFrontmatter` all resolve through `useSpaceFS()` (`sdk/org/libs/state/src/hooks/fs/useFile.ts#useFile`, `useGlob.ts:10-11`). So the *reads* in `useProjectConfig()` (`useFile('lmthing.json')`, `sdk/org/libs/state/src/hooks/project/useProjectConfig.ts#useProjectConfig`), `useProjectEnv()` (`sdk/org/libs/state/src/hooks/project/useProjectEnv.ts#useProjectEnv`), `useProjectEnvList()` (`useProjectEnvList.ts:9-12`) and the `workspace/` env hooks (`useEnvFile.ts:8-10`, `useEnvFileList.ts:6-7`) hit `<projectId>/<spaceId>/lmthing.json` and `<projectId>/<spaceId>/.env*`, **not** the project-root keys `P.projectConfig`/`P.projectEnv` describe — and since `SpaceProvider` never hydrates `.env*` into the cache (`isRunnableSpaceFile`, `sdk/org/libs/state/src/lib/pod/transport.ts#isRunnableSpaceFile`), those env reads return `null` in practice. The *writers* are correct: `useUpdateProjectConfig` and `useProjectEnvWritable` go through `projectFS` (`useProjectConfig.ts:36-54`, `useProjectEnv.ts:32-68`). Use `useProjectFS()` directly for project-root files.
