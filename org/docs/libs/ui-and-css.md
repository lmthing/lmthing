...
Allowed (not flagged): token-based `rgb/hsl(var(--…))`, and **achromatic** overlays/scrims/shadows (grey/black/white) with alpha < 1 (`funcAllowed`, `lint-design-tokens.mjs:44-57`). Comments are not scanned at all (`lint-design-tokens.mjs#stripComments`) — prose about a colour is not a colour. Escape hatches: `ds-lint-ok` on a line skips that line; `ds-lint-file-ok` anywhere skips the whole file — for terminal ANSI palettes, syntax themes. The token-definition files themselves (`theme.css`, `tokens.json`, `tokens.manifest.json`, anything under `scripts/`) are always exempt (`ALLOW_FILE`, `lint-design-tokens.mjs:30-34`).
