...
**`COMMON_DTS`** — declarations present in **every** VM context (session, fork leaf, delegate): `display`, `setActivity`, `inspect`, `loadKnowledge`, `sleep`, the `JSXDescriptor`/`ForkOpts`/`DelegateOpts` interfaces, the classic-JSX `React` and `JSX` namespaces, and host-injected primitives (`readDocument`, `integrationStatus`, `typecheckSource`, `spacePath`, `resolveSpaceDir`, `progress`), plus `catalogDts()` (the design-system catalog as typed JSX globals) appended at the end `sdk/org/libs/core/src/typecheck/library-dts.ts:50-121`. What earns a place here is being injected **unconditionally** by `injectHostTools` — a global whose injection is gated gets its own fragment, gated the same way. `readFileRaw`/`writeFileRaw` have no fragment at all, `registerSpace` has `REGISTER_SPACE_DTS`, and `fetch`/`process` are deliberately absent — see the paragraphs below.

**`REGISTER_SPACE_DTS`** — `registerSpace()` `sdk/org/libs/core/src/typecheck/library-dts.ts#REGISTER_SPACE_DTS`, emitted by `buildAmbientDts` only under `caps.registerSpace` `sdk/org/libs/core/src/exec/bootstrap.ts:396-400`, matching the gate on its injection `sdk/org/libs/core/src/exec/bootstrap.ts:269`.

> It used to live in `COMMON_DTS`, declared unconditionally "matching the pre-unification DTS, where only ask/tasklist/fork/delegate were stripped" — while injection was already gated. That broke the invariant in the direction that costs something: in a delegate and in a read-only fork the global is not injected, so `await registerSpace(dir)` **typechecked and then threw at runtime** instead of failing typecheck cleanly and retryably. Only the injection half was ever tested (`sdk/org/libs/core/src/fork/fork.test.ts:414`), which is how the declaration half drifted unnoticed. Both halves are now pinned per context in `sdk/org/libs/core/src/exec/bootstrap.test.ts`.

The grant itself also narrowed: `registerSpace` is now held **only by write-capable fork roles** `sdk/org/libs/core/src/exec/capability.ts#forkCapabilities` — not by sessions `sdk/org/libs/core/src/exec/capability.ts#sessionCapabilities` and not by delegates `sdk/org/libs/core/src/exec/capability.ts#delegateCapabilities`. That is where the only callers in the shipped spaces run: the architect's `register` and `reregister` nodes, both `role: general` (`sdk/org/libs/core/system-spaces/system-architect/tasklists/synthesize_and_run/07-register.md`, `sdk/org/libs/core/system-spaces/system-architect/tasklists/iterate_space/04-reregister.md`). A top-level orchestrator that cannot scaffold a space can no longer register one either.

**Raw fs/shell primitives** — `readFileRaw` and `writeFileRaw` have **no DTS fragment anywhere**. They are internal-only host primitives (memory/todos and the architect's builder functions call them in bodies that are not typechecked against any DTS), so nothing can name them and typecheck. `execShell` has exactly ONE declaration site: `EXEC_SHELL_DTS` `sdk/org/libs/core/src/typecheck/library-dts.ts#EXEC_SHELL_DTS`, emitted with `SCRATCH_DTS`'s `createScratch` `sdk/org/libs/core/src/typecheck/library-dts.ts#SCRATCH_DTS` only for an agent holding `fs:scratch` (the engineer's scratch sandbox), where `execShell` is the scratch-rooted variant `sdk/org/libs/core/src/exec/bootstrap.ts:416-417`. Generic filesystem is off every other agent's model surface — a stray `readFile`/`writeFile`/`execShell` fails typecheck, and persistence goes through the typed `writeProject*`/architect builder functions instead.

**`fetch` and `process` — split, not uniformly internal-only.** Both are injected (real implementations) into every VM unconditionally by `injectHostTools`/`createChildVM`, but they no longer share one declaration story:
- `fetch` stays **internal-only on every model surface**: `NET_FETCH_DTS` `library-dts.ts#NET_FETCH_DTS` is appended only to the `LIBRARY_DTS`/`LIBRARY_DTS_NO_ASK` bundles (which `typecheckSource` uses to check system-function BODIES — `webSearch`/`webFetch` call raw `fetch`), never by `buildAmbientDts`. A model-authored `fetch(...)` fails typecheck in every context — the same treatment as `readFileRaw`/`writeFileRaw`.
- `process` **splits in two**. `process.env` gets the `fetch` treatment: `PROCESS_ENV_DTS` `library-dts.ts#PROCESS_ENV_DTS` (both `env` and `exit` members) lives only in the `LIBRARY_DTS` bundles, for function bodies like `webSearch` reading `TAVILY_API_KEY` — never declared for the model, closing a secrets-leak path. `process.exit` does NOT get that treatment: it is intentional-termination control flow with no secret, so `buildAmbientDts` appends a separate, minimal, env-free `PROCESS_EXIT_DTS` `library-dts.ts#PROCESS_EXIT_DTS` **unconditionally**, the same tier as `COMMON_DTS` `bootstrap.ts:392-400` — every model surface declares `process.exit` (see the retry-on-type-error path below), but none declares `process.env`.

The two full bundles are the internal-only surface — what `typecheckSource` checks a standalone space-function source against, via `runTsc` `sdk/org/libs/core/src/globals/host-tools.ts:266`:
- `LIBRARY_DTS` — everything incl. `ask` `sdk/org/libs/core/src/typecheck/library-dts.ts:446`. The only bundle still used at runtime.
- `LIBRARY_DTS_NO_ASK` — no `ask` (fork/delegate VMs are headless; a stray `await ask(...)` there fails typecheck with `Cannot find name 'ask'` instead of blocking forever on stdin) `sdk/org/libs/core/src/typecheck/library-dts.ts:455`. `buildAmbientDts` composes fork/delegate DTS additively and does not consume this bundle; it survives as an export referenced only by tests.

> They used to re-append `WRITE_PRIMITIVES_DTS = [EXEC_SHELL_DTS, WRITE_FILE_RAW_DTS, READ_FILE_RAW_DTS]` so `typecheckSource` saw the FULL global set. That bought nothing: `typecheckSource` **drops** `Cannot find name` diagnostics (TS2304/2552), because one space function may legitimately reference siblings absent from an isolated check `sdk/org/libs/core/src/globals/host-tools.ts:264-272` — so a builder body naming an undeclared `execShell` was never going to be reported either way. What the re-append did do was keep three raw fs/shell primitives alive in a bundle the package index exports. The fragments are gone; `EXEC_SHELL_DTS` survives for the `fs:scratch` gate alone.
...
- **`db.*`** — the three `db:*` verbs share ONE `db` object, so they cannot be three separate `declare const db` blocks. `composeDbDts({ read, write, schema }, tables?)` unions the present member strings (`DB_READ_MEMBERS` / `DB_WRITE_MEMBERS` / `DB_SCHEMA_MEMBERS`) into a single `declare const db`, returning `''` when none are present (so a stray `db` call fails typecheck) `library-dts.ts#composeDbDts`. In the agent sandbox `db.*` is a **synchronous** host call (non-Promise), unlike the value-yielding `apiCall`.
  - **Schema-gated table/column names.** When the real per-run schema is threaded in as `tables` (`DbTableSchema[]` — each table's name + its column names, derived cheaply from the project's `database/<name>.json` basenames + column keys, NOT `ts-json-schema-generator`) `library-dts.ts#DbTableSchema` AND the agent is **not** a schema author (`!present.schema`), the read/write members are emitted in their GATED form `library-dts.ts#DB_READ_MEMBERS_TYPED` · `library-dts.ts#DB_WRITE_MEMBERS_TYPED`: a `type __DbCols = { "<table>": "<col>" | … }` map plus `query`/`insert`/`update`/`remove` written as `<T extends keyof __DbCols>(table: T, … Partial<Record<__DbCols[T], unknown>> …)`. So a hallucinated **table** (`T` not in `keyof __DbCols`) or a typo'd **column** (an excess property on `where`/`set`/`values`, or a bad `orderBy`) FAILS typecheck — a clean, retryable, model-visible error whose message names the real columns — instead of returning an empty result the model then fabricates against. A dynamic or `any`-typed table arg still resolves (`T ⇒ any`), and `tables()` yields the `keyof __DbCols` union so an enumerate-then-query loop still typechecks; only NAMES are gated, never value types (they stay `unknown`).
  - **Loose fallback (unchanged pre-change behavior).** A **db:schema holder** creates tables mid-session (an open table set — the automator/data-modeler), so it always keeps the loose `string`-typed members `library-dts.ts#DB_READ_MEMBERS` · `library-dts.ts#DB_WRITE_MEMBERS`; likewise `createTable`/`addColumn` `library-dts.ts#DB_SCHEMA_MEMBERS`, and any session with no threaded schema (a non-project session, or a path that does not thread it) stays loose — a bad table/column there still throws at runtime as before, not at typecheck.
...
5. `caps.registerSpace ? REGISTER_SPACE_DTS : ''` — write-capable fork roles only `sdk/org/libs/core/src/exec/bootstrap.ts:396-400`
6. `COMMON_DTS` (unconditional)
7. `PROCESS_EXIT_DTS` (unconditional, same tier as `COMMON_DTS`) — the env-free `process.exit` fragment `bootstrap.ts:392-400`; deliberately not `PROCESS_ENV_DTS` (`process.env` stays off every model surface)
8. `caps.scratchFs ? EXEC_SHELL_DTS : ''` and `caps.scratchFs ? SCRATCH_DTS : ''` — the engineer's scratch sandbox ONLY; `writeFileRaw`/`readFileRaw` have no fragment at all `sdk/org/libs/core/src/exec/bootstrap.ts:416-417`
9. `buildAppCapabilityDts(caps.app, opts.appDts, opts.projectRoot)` — the app-capability fragments
10. `opts.overlay ?? ''` — the function/component overlay
11. `opts.currentTask ? CURRENT_TASK_DTS : ''` — the fork/delegate `currentTask.resolve()` capture global `bootstrap.ts:270`
12. `...(opts.extraDecls ?? [])` — fork seed/upstream vars, delegate query/context
