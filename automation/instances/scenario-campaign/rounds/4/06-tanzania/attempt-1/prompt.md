...
2. **Launch the replay in the BACKGROUND, capturing its PID.** The full run is 30–40 min — longer
   than the 10-min Bash-tool cap — so it CANNOT be one foreground call. The runner wipes the pod
   runtime root, starts a clean server, provisions the one project, and writes `step-NN.json` +
   appends `trace.md` after EACH step:

       ART=<ARTIFACT_DIR>   # the dir holding /home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/4/06-tanzania/PROGRESS.md; evidence + your report live here
       cd sdk/org && SCENARIO_TARGET=local nohup node scenarios/run-yaml.mjs 06-tanzania \
         --fresh-server --out "$ART" > "$ART/run.log" 2>&1 & echo $! > "$ART/runner.pid"
...
5. **Verify-rerun** (after a fix): rebuild if you touched code (`pnpm --filter @lmthing/core... build`
   ...
   then relaunch the SAME background+poll but only THROUGH the failed step:
       ...
       cd sdk/org && SCENARIO_TARGET=local nohup node scenarios/run-yaml.mjs 06-tanzania \
         --fresh-server --through <N> --out "$ART" > "$ART/run.log" 2>&1 & echo $! > "$ART/runner.pid"
...
## The report (write it as `report.md` in the same directory as your progress log `/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/4/06-tanzania/PROGRESS.md`)

There is NO per-scenario `results/` dir and NO `run.mjs` — a scenario is just `scenario.yaml` +
`fixtures/`, played by the generic runner. Your report lives beside `/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/4/06-tanzania/PROGRESS.md` in the round's
