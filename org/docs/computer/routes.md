...
- `/computer/login` is orphaned. It has no inbound reference anywhere in the repo: no nav item lists it, nothing links or redirects to it, and its only occurrence outside the generated route tree (`sdk/org/apps/web/src/routeTree.gen.ts:730-736`) is its own `createFileRoute('/computer/login')` `sdk/org/apps/web/src/routes/computer/login.tsx#Route`. Unauthenticated users never reach it either, because `AuthGate` redirects them to lmthing.com to sign in before any child route mounts `sdk/org/apps/web/src/lib/gates.tsx#AuthGate`. It is reachable only by typing the URL, and it renders nothing when you do `sdk/org/apps/web/src/routes/computer/login.tsx#Login`.
