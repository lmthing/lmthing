...
The CI bundle step prepends `C:\Windows\System32` to `PATH` on Windows so `build.mjs` uses the system `tar`: Git Bash tar interprets a `D:\...` archive path as a remote-spec hostname and fails before compression starts `.github/actions/build-cli-bundle/action.yml:55-68`.
...
| **Some packages are unscannable by construction** | `react-dom` is resolved by name from an *array of strings* — the `--web` server loops over `['react', 'react-dom', 'react/jsx-runtime', 'react-dom/client']` calling `req.resolve(pkg)` to pin one React instance `sdk/org/libs/cli/src/web/serve.ts#resolveUiAssets`. No scan of import syntax can see that, and its absence breaks the `--web` DevTools UI rather than startup `sdk/org/libs/cli/scripts/bundle/payload.mjs#UNSCANNABLE`. |
