...
- **`model`** — optional per-agent model alias/spec, trimmed and kept only when a non-empty string `sdk/org/libs/core/src/spaces/load.ts:491`. It takes effect **wherever that agent runs**: when it is delegated to, `runDelegate` runs its turns on `agent.model ?? opts.model` `sdk/org/libs/core/src/delegate/delegate.ts:182-184`; when it is the session's TOP-LEVEL agent, `start()`/`resume()` resolve the session's own turns to the same `agent.model ?? opts.modelAlias` `sdk/org/libs/core/src/session/session.ts:399-402` · `sdk/org/libs/core/src/session/session.ts:606-609`. So a specialist can demand its own model regardless of the caller's, and a space can pin itself strong while the pod default stays small. (Top-level honouring is new — until then `model:` moved the needle only on the delegate path.) That effective model is also the session's fork `defaultModel` and the model delegates inherit `sdk/org/libs/core/src/session/session.ts:1113`, so a fork or delegate that declares nothing follows the running agent. `system-vision`'s agent uses exactly this (`model: vision`) `sdk/org/libs/core/system-spaces/system-vision/agents/vision/instruct.md:1-5`. The string is resolved lazily per turn — first as an alias via `LM_MODEL_<NAME>`, otherwise taken as a literal spec `sdk/org/libs/cli/src/cli/bin.ts:316-329` · `sdk/org/libs/cli/src/providers/aliases.ts#resolveAlias`.
