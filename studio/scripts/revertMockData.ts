  ...
  for (const workspace of workspaces) {
    ...
    try {
      ...
      for (const agentId of agents) {
        ...
        const agent: any = {
          ...
          enabledKnowledgeFields: frontmatter.enabledKnowledgeFields || [],
          runtimeFields: configData.runtimeFields || [],
        ...
        const slashRegex =
          /<slash_action name="([^"]*)" description="([^"]*)" flowId="([^"]*)">\n\/([^\n]+)\n<\/slash_action>/g;
        let match;
    ...
    try {
      const pkgJson = JSON.parse(
        await fs.readFile(path.join(inWorkspacePath, "package.json"), "utf8"),
      );
    ...
    if (agentBuilderData.savedAgentConfigs.length > 0 || agentBuilderData.toolLibrary.length > 0) {
      ...
      await fs.writeFile(
        path.join(agentBuilderOutDir, "data.json"),
        JSON.stringify(agentBuilderData, null, 2),
      );
    }
    ...
    if (flowBuilderData.flows.length > 0) {
      ...
      await fs.writeFile(
        path.join(flowBuilderOutDir, "data.json"),
        JSON.stringify(flowBuilderData, null, 2),
      );
    }
    ...
    async function readPromptNode(
      ...
      node.children.sort((a: any, b: any) => {
        const orderA =
          a.frontmatter?.order ?? (a.type === "directory" ? -1 : Number.MAX_SAFE_INTEGER);
        const orderB =
          b.frontmatter?.order ?? (b.type === "directory" ? -1 : Number.MAX_SAFE_INTEGER);
    ...
    if (promptLibSystem) {
      ...
      await fs.writeFile(
        path.join(promptLibOutDir, "data.json"),
        JSON.stringify({ fileSystem: promptLibSystem }, null, 2),
      );
    }
  }
