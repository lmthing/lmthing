...
`INTEGRATION_SMS_ACCOUNT_SID` + `INTEGRATION_SMS_AUTH_TOKEN` (set in **the project's Settings → Integrations**). You
...
- `from` must be one of the user's OWN Twilio numbers — normally their `INTEGRATION_SMS_FROM_NUMBER`. Pass it
...
"not configured — set INTEGRATION_SMS_AUTH_TOKEN in Settings → Integrations"). In that case, do NOT retry
