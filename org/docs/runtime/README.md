...
`@lmthing/core` (`sdk/org/libs/core/src/index.ts`) is the whole agent runtime: a QuickJS WASM sandbox, the streaming statement pipeline, the yield protocol, spaces/forks/delegates, budgets and the trace spine. It has **no** provider and **no** renderer — the host passes in a `streamFn` (`sdk/org/libs/core/src/session/types.ts:L113-L115`) and a `RenderHost` (`display` / `ask` / `log`, `sdk/org/libs/core/src/session/types.ts:L10-L14`).
...
`runTurnLoop` (`sdk/org/libs/core/src/eval/turn-loop.ts:L300-L770`) is the loop. One iteration of its `while (attempt < maxRetries)` is one LLM request:
...
`createVM` (`sdk/org/libs/core/src/sandbox/quickjs.ts:L61-L250`) builds one QuickJS runtime + context per VM, with a **64 MB memory limit** and a **5 s per-statement interrupt deadline** (`quickjs.ts:L62-L63`, `L92`).
...
```ts
// sdk/org/libs/core/src/eval/yield.ts:L3-L8
...
`ask` is the shape of them all: validate, `pushYield`, return a promise the host settles (`sdk/org/libs/core/src/globals/ask.ts:L64-L92`). The VM suspends because its module promise is pending; `evalStatement` returns as soon as `pendingYields` is non-empty (`quickjs.ts:L114-L119`).
...
**Routing.** `routeCommonYield` (`sdk/org/libs/core/src/eval/yield-router.ts:L131-L374`) is the single resolver shared by the session, delegate and fork-leaf VMs: `sleep`, `fork`, `tasklist`, `delegate`, `fetch`, `apiCall`, `callConnection`, `tool`, `readDocument`, `integrationStatus`, `consent`, `storeSearch`/`storeInspect`/`installSpace`, `emitEvent`, plus `loadKnowledge`/`registerSpace` for fork leaves. Kinds it returns `{handled:false}` for (`ask`, `inspect`, and the session's own `loadKnowledge`/`registerSpace`/`setSessionMeta`) are handled by the caller — `Session.handleYield` (`sdk/org/libs/core/src/session/session.ts:L795-L851`). A missing host resolver throws an actionable error instead of binding `undefined` (e.g. `yield-router.ts:L194-L196`, `L338-L340`).
...
**What the model sees next.** The loop appends a `VARIABLES` message (`turn-loop.ts:L718-L736`) built by `emitVariables` (`sdk/org/libs/core/src/context/variables.ts:L9-L28`) — the bound names (serialized, lossy previews), a `SCOPE (already declared — do not redeclare)` list and the full `ALREADY EXECUTED` block. Folded in on top:
...
The idle watchdog cannot fire while a *synchronous* host call (e.g. `execShell`, `sdk/org/libs/core/src/globals/host-tools.ts:L78`) blocks the Node event loop; `fetch` is a real non-blocking yield, so it is not in that category (`yield-router.ts:L184-L189`).
...
`CapabilityProfile` (`sdk/org/libs/core/src/exec/capability.ts:L47-L79`) is the single description of what a VM context may do: `ask`, `orchestrate` (`fork`+`tasklist`), `delegate`, `registerSpace`, `setSessionMeta`, `allowWrite`, and the parsed project-app grants `app` (`capabilities:` frontmatter). Three constructors:

- `sessionCapabilities` — full toolkit incl. interactive `ask` (`capability.ts:L84-L86`);
...
`Session` (`sdk/org/libs/core/src/session/session.ts:L89-L990`) owns the VM, history, spaces, budget and tracer:
...
- **Soft todos.** `beforeTurn` re-injects open items from `<spaceDir>/.lmthing/todos.json` into **every** top-level turn as a transient (never persisted) user message (`session.ts:L777-L793`, consumed at `turn-loop.ts:L345-L349`). Forks/delegates do not set it.
...
`Budget` (`sdk/org/libs/core/src/eval/budget.ts:L46-L137`) is a **host-side** counter — set by the caller (Session / ForkEngine), never reachable from inside the VM, so a model cannot lift its own ceiling (`budget.ts:L1-L14`). Four coarse limits (`BudgetLimits`, `budget.ts:L16-L25`):
...
Breaching any limit throws `BudgetExceededError` (`budget.ts:L29-L38`), which propagates out of the turn loop for the caller to dispose the VM on. Before the hard stop, `nearLimitWarning()` (≤2 turns/tool-calls remaining, or 80 % of the wall clock) is appended to the VARIABLES block so the model can wrap up and `currentTask.resolve()` (`budget.ts:L108-L131`, `turn-loop.ts:L734-L735`). The VM can *read* its own spend through the `progress()` host tool (`session.ts:L651`, `globals/host-tools.ts:L26-L29`). A fresh `Budget` is minted per `start()`/`continue()`/`resume()` (`session.ts:L203`, `L304`, `L487`) and per fork (`fork/fork.ts:L231`).

---

## 9. The statement protocol (what the model is told)

`STATEMENT_PROTOCOL` (`sdk/org/libs/core/src/exec/preamble.ts:L19-L41`) is injected **once per context** at the top of every system prompt (session/delegate via `buildSystemBlock`, `sdk/org/libs/core/src/context/system-block.ts:L124`, `L209`). Its load-bearing rules mirror the runtime exactly:
