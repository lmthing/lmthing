...
The same patterns are matched **client-side** at runtime: `matchRoutes` splits pattern and path into segments and captures `:param`s `sdk/org/libs/cli/src/app/runtime/router.tsx:55-74`; the matched page renders inside `_layout` inside `_app` `sdk/org/libs/cli/src/app/runtime/router.tsx:167-205`; an unmatched path renders a minimal `No page for <path>` `sdk/org/libs/cli/src/app/runtime/router.tsx:207-214`. There is no pod-side page routing beyond the SPA fallback below.
