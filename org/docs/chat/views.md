...
| `AppShell` | `app/AppShell.tsx` | Top bar over a main pane, no left sidebar: TopBar ∣ (routed pane ∣ NoSessionPane ∣ ChatView) ∣ DevPanel (+ ProjectSettings drawer) |
...
## Opening a project's chat

A project's main surface is its chat, so a bare `/chat/<project>` does not sit on an empty pane: `ChatShell` resolves the project's most-recent conversation (or starts a fresh one when it has none) and **redirects** to `/chat/<project>/<conversation>`, where `ChatView` renders `sdk/org/libs/ui/src/chat/app/session-control.ts#resolveProjectChat` · `sdk/org/libs/ui/src/chat/app/ChatShell.tsx#ChatShell`. The redirect *replaces* the bare-project entry (the app answering "open this project", not a place to come Back to), and the main pane shows `OpeningPane` for the brief window before it lands. The resumed id is handed back for the shell's location effect to open, so a cold pod (`503`) or a deleted snapshot flows through the same opening / gone / unavailable panes as a pasted conversation link `sdk/org/libs/ui/src/chat/app/RoutePanes.tsx#OpeningPane`.
