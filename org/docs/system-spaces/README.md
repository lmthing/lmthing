...
|---|---|---|
| `system-appbuilder/automator` | `hooks:write`, `db:schema`, `db:read`, `db:write`, `api:write`, **`views:write`** | every LIVE-project writer except the TSX pair — `writeProjectTable`/`Api`/`Hook`/`Event`/`Function` + `db.*`, plus the spec writers `writeProjectView`/`writeProjectViewComponent`/`writeProjectViewShell`. **`pages:write` is absent**, so `writeProjectPage`/`writeProjectComponent` are in neither the injected globals nor the DTS (`system-appbuilder/agents/automator/instruct.md:L9-L15`, `:L44-L46`) |
