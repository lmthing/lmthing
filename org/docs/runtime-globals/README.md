...
| `listProjectDir(dir)` / `readProjectFile(path)` | S | List/read project files, rooted at `projectRoot` — the **only** way an agent reads project files now. `readProjectFile` returns the **plain** text in `.content` (NO `.raw`, NO line-numbered variant — unlike the engineer's scratch `readFile`) | `projectRoot` set (**not** a capability) | [app-authoring.md](./app-authoring.md) · [session-and-utils.md](./session-and-utils.md#readprojectfilepath--readspacefilespace-relpath--plain-text-in-content-no-raw) · [../format/project/README.md](../format/project/README.md) |
