...
- **The reserved `system` project id.** `<root>/system/spaces/<id>` matches the generic `<root>/<projectId>/spaces/<id>` shape, so Studio edits system spaces through the ordinary project routes `sdk/org/libs/cli/src/server/projects.ts:10-16`. It is a route alias, not a project: `listProjects` never returns it `sdk/org/libs/cli/src/server/projects.ts#listProjects`, and it cannot be created or deleted `projects.ts:31-41`, `projects.ts:330-336`.
