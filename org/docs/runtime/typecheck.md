...
**`COMMON_DTS`** — declarations present in **every** VM context (session, fork leaf, delegate): `console` (injected host substrate — declared here because the checker runs with `types: []`, so nothing may lean on an ambient `@types/node`), `display`, `inspect`, `loadKnowledge`, `sleep`, the `JSXDescriptor`/`ForkOpts`/`DelegateOpts` interfaces, the classic-JSX `React` and `JSX` namespaces, and host-injected primitives (`readDocument`, `integrationStatus`, `typecheckSource`, `spacePath`, `resolveSpaceDir`, `progress`), plus `catalogDts()` (the design-system catalog as typed JSX globals) appended at the end `sdk/org/libs/core/src/typecheck/library-dts.ts#COMMON_DTS`. Everything in it is injected **unconditionally** by `injectHostTools`, which is what earns it an unconditional declaration; a global whose injection is gated gets its own fragment, gated the same way. `fetch` and `process` are deliberately **not** in this list — see the internal-only-primitives paragraph below.

> **`REGISTER_SPACE_DTS`** — `registerSpace` used to sit in `COMMON_DTS`, declared unconditionally "matching the pre-unification DTS, where only ask/tasklist/fork/delegate were stripped", while its injection was already gated. That broke the invariant this whole registry exists to hold: in a delegate and in a read-only fork the model could write `await registerSpace(dir)`, watch it typecheck, and take a runtime throw. It is now its own fragment (`sdk/org/libs/core/src/typecheck/library-dts.ts#REGISTER_SPACE_DTS`) emitted under the same `caps.registerSpace` flag that gates the injection (`sdk/org/libs/core/src/exec/bootstrap.ts:L460-L465`), and the grant narrowed to **write-capable fork roles** — no session, no delegate (`sdk/org/libs/core/src/exec/capability.ts#sessionCapabilities`). An injected-iff-declared assertion per context pins the halves together (`sdk/org/libs/core/src/exec/bootstrap.test.ts`).

**Raw fs/shell primitives** — split OUT of `COMMON_DTS` and, critically, **NO LONGER emitted on any agent's model DTS by default**. `readFileRaw`/`writeFileRaw` now have **no DTS fragment anywhere**: they are internal-only host primitives — memory/todos and the architect's builder functions call them in bodies that are NOT typechecked against any DTS — so nothing can name them and typecheck. `execShell` has exactly ONE declaration site, `EXEC_SHELL_DTS` (`sdk/org/libs/core/src/typecheck/library-dts.ts#EXEC_SHELL_DTS`), emitted only for an agent holding `fs:scratch` (the engineer's scratch sandbox), where it is the scratch-rooted variant, alongside `SCRATCH_DTS`'s `createScratch` (`sdk/org/libs/core/src/typecheck/library-dts.ts#SCRATCH_DTS`). Generic filesystem is off every other agent's model surface — a stray `readFile`/`writeFile`/`execShell` fails typecheck, and persistence goes through the typed `writeProject*`/architect builder functions instead.
...
The two full bundles used to re-append `WRITE_PRIMITIVES_DTS = [EXEC_SHELL_DTS, WRITE_FILE_RAW_DTS, READ_FILE_RAW_DTS]` so their one runtime consumer — `typecheckSource`, which checks a standalone space-function source — saw the FULL global set. **That bought nothing and has been dropped**: `typecheckSource` already discards `Cannot find name` diagnostics (TS2304/TS2552), because an isolated function may legitimately reference siblings absent from the check, so an undeclared `execShell(...)` in a builder body was never reported either way. What the re-append DID do was keep three raw fs/shell primitives alive in a bundle the package index exports. The bundles now carry `REGISTER_SPACE_DTS` and no raw primitives:
- `LIBRARY_DTS` — everything incl. `ask` `sdk/org/libs/core/src/typecheck/library-dts.ts#LIBRARY_DTS`. The only bundle still used at runtime.
...
- **`views:write`** → `PROJECT_VIEW_DTS` — the **only** UI-authoring surface: `writeProjectView`/`writeProjectViewLayout`/`writeProjectViewComponent`/`writeProjectViewShell`. A page is validated spec data (`views/<route>.view.json`) rendered by the shared `ViewRenderer`, never hand-authored TSX; the model-facing freehand-TSX writers (`writeProjectPage`/`writeProjectComponent`) and their `pages:write` capability id were removed entirely, so there is no TSX-page DTS fragment left to grant `library-dts.ts#PROJECT_VIEW_DTS` · `library-dts.ts#CAPABILITY_DTS_FRAGMENTS`.
...
6. `PROCESS_EXIT_DTS` (unconditional, same tier as `COMMON_DTS`) — the env-free `process.exit` fragment `bootstrap.ts:392-400`; deliberately not `PROCESS_ENV_DTS` (`process.env` stays off every model surface)
7. `caps.scratchFs ? EXEC_SHELL_DTS : ''` and `caps.scratchFs ? SCRATCH_DTS : ''` — the engineer's scratch sandbox ONLY; `writeFileRaw`/`readFileRaw` are never emitted on the model DTS `bootstrap.ts:354-361`
8. `buildAppCapabilityDts(caps.app, opts.appDts, opts.projectRoot)` — the app-capability fragments
9. `opts.overlay ?? ''` — the function/component overlay
10. `opts.currentTask ? CURRENT_TASK_DTS : ''` — the fork/delegate `currentTask.resolve()` capture global `bootstrap.ts:270`
11. `...(opts.extraDecls ?? [])` — fork seed/upstream vars, delegate query/context
