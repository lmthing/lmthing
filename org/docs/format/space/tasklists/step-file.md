...
> This was declarative-only until 2026-07-20 — parsed and then read by nothing. A node could declare an input its dependencies never emit, run anyway, receive `undefined`, and fail somewhere downstream with an error naming the wrong node. Dead contract metadata is worse than none: a reader wiring a new node into the DAG relies on it as a guarantee.
