...
selectedDomains: ["domain-{domain1}", "domain-{domain2}", "domain-{domain3}"]
---

<slash_action name="{Action Name}" description="{What this action does}" flowId="flow_{action_id}">
/{command}
</slash_action>
```
...
- `selectedDomains`: Array of domain references prefixed with "domain-"
...
- [ ] Agent selectedDomains reference existing knowledge domains
...
| Agent Config | `agents/agent-*/config.json` | JSON | emptyFieldsForRuntime |
| Agent Instruct | `agents/agent-*/instruct.md` | MD+YAML | name, description, tools, selectedDomains |
