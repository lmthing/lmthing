...
| 4a′ | **A CHANGED FACT about data already in the app** | "I renewed the car insurance, the new policy number is AX-7741-2", "the rent went up to €900", "mark that invoice paid" | the automator too — it is the only agent holding `db:write`. **Not** the domain space: `household-insurance-admin` and friends READ their knowledge and REPLY (their `answer` tasklist cannot write the db), so an update routed there yields a fluent confirmation and changes nothing. Route on the **intent, not the words** — the Greek twin of the sentence takes the same path (live in scenario 07, the English update wrote the row and the Greek one was answered in prose) (`sdk/org/libs/core/system-spaces/user-thing/agents/thing/instruct.md:L255-L272`) |
