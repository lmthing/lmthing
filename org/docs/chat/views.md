...
Header: session title, live session cost (`sessionCostUsd + sessionCostInflight`), follow toggle, `ConnectionDot`, `TraceLoader`, Inspect, Report bug, theme toggle, and a restart button `sdk/org/libs/ui/src/chat/app/ChatView.tsx:86-97,178-239`. The title prefers the agent-set session title (from `setSessionMeta`, delivered as a `session_meta` trace event) and falls back to `<project|space> · <Agent>` `sdk/org/libs/ui/src/chat/app/ChatView.tsx:157-173`. So a new chat is never title-less, the store seeds `sessionTitle` from the **first user message** (whitespace-collapsed, capped 80 chars) the moment it is sent, as a client placeholder — set by `noteUserMessage` only while no title exists yet, and overridden by a later agent `session_meta` `sdk/org/libs/ui/src/chat/store/session-slice.ts:156`. Directly under the title the header shows the agent's live `setActivity` "currently doing" status — **only** the top-level session (THING) line, cleared when the turn goes idle `sdk/org/libs/ui/src/chat/app/ChatView.tsx:187-197`. A fork/delegate `setActivity` is **not** shown here; it sets that work node's narration in the `LiveActivity`/`WorkBlock` sub-agent panel instead (see [Sub-agent activity](#sub-agent-activity)) ([../runtime-globals/session-and-utils.md](../runtime-globals/session-and-utils.md)).
