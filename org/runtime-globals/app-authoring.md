...
**Read-only fork roles (`explore`/`plan`) lose every authoring grant.** `intersectAppCaps(app, allowWrite=false)` keeps only `db:read`, `api:call`, `connections:use`, `tools:use`, `store:read`; `db:write`, `db:schema`, `pages:write`, `api:write`, `hooks:write`, `store:install`, `events:emit` are dropped before the profile is built `sdk/org/libs/core/src/exec/capability.ts:16-28` · `sdk/org/libs/core/src/exec/capability.ts:94-97`.
...
| Needs a selected project? | Yes — `createProject`/`selectProject` first (`requireCurrent()` throws otherwise) `sdk/org/libs/cli/src/app/authoring/globals.ts:138-143` | No — bound to one fixed project root at construction `sdk/org/libs/cli/src/app/authoring/globals.ts:300-312` |
...
| Present when? | Always (the pod always constructs them) `sdk/org/libs/cli/src/server/session-manager.ts:610-621` | Only in a project-rooted session (that is where the host supplies the impls) `sdk/org/libs/cli/src/server/session-manager.ts:559-630` |

The catalog root is `<monorepoRoot>/store/projects` (or `LM_STORE_APPS_DIR`), always `mkdir -p`'d before use `sdk/org/libs/cli/src/app/authoring/catalog-root.ts:24-36`.
...
- `writeProjectPage` / `writeProjectApi` fire `onAppWrite`, which drops the project's typed endpoint contracts and disposes its API runtime. **Page compilation is deliberately NOT done on write** — the caller must `POST /api/projects/:projectId/app/build` afterwards `sdk/org/libs/cli/src/server/session-manager.ts:591-609` (see [../cli-api/rest/projects.md](../cli-api/rest/projects.md)).
...
**Typed overloads replace the generic signature.** When the host supplies project-generated contracts, `buildAppCapabilityDts` uses them *instead of* the generic `API_CALL_DTS` `sdk/org/libs/core/src/exec/bootstrap.ts:292-295` · `sdk/org/libs/core/src/typecheck/library-dts.ts:160`. They are built with `fallback: false`, so a wrong endpoint name **and** a wrong input type are both hard typecheck errors inside the sandbox `sdk/org/libs/cli/src/app/build/apicall-dts.ts:28-47` · `sdk/org/libs/cli/src/app/build/contracts.ts:23-33`. One overload per endpoint:
...
In the pod the resolver is attached only when `getApiRuntime` returns a runtime, and that returns `null` for a project with no db `sdk/org/libs/cli/src/server/session-manager.ts:610-617` · `sdk/org/libs/cli/src/server/session-manager.ts:669-674`.
...
- **The catalog writers do NOT parse-check `src`.** `assertSourceParses` is called only by the live-project writers; `writePage`/`writeApi`/`writeHook` write whatever string they are handed `sdk/org/libs/cli/src/app/authoring/globals.ts:206-249` vs `sdk/org/libs/cli/src/app/authoring/globals.ts:332-456`.
- **DTS and injection are not in lockstep for the live twins.** `PROJECT_PAGE_DTS`/`PROJECT_API_DTS`/`PROJECT_AUTHORING_DTS`/`PROJECT_TABLE_DTS` are appended to their capability's fragment unconditionally `sdk/org/libs/core/src/typecheck/library-dts.ts:266-275` · `sdk/org/libs/core/src/exec/bootstrap.ts:291`, while injection additionally requires the host impl `sdk/org/libs/core/src/exec/app-globals.ts:204-218`. In a session where the host supplies no live impls (e.g. a non-project appbuilder run), `writeProjectPage(...)` passes typecheck and fails at runtime.
- **`api:call`'s `allow` list is not enforced at call time.** It is required at parse time and stored on the grant `sdk/org/libs/core/src/spaces/capabilities.ts:96-98` · `sdk/org/libs/core/src/spaces/capabilities.ts:317-323`, but the typed DTS is generated from **every** endpoint in the project (`generateProjectContracts` → `buildApiCallDts(endpoints)`, no filter) `sdk/org/libs/cli/src/app/build/contracts.ts:23-33`, and the pod resolver dispatches by name with no allow-list check `sdk/org/libs/cli/src/server/session-manager.ts:217-229`. The filter `buildApiToolSignatures(endpoints, allow)` exists `sdk/org/libs/cli/src/app/build/apicall-dts.ts:61-80` but has no non-test caller (`rg buildApiToolSignatures libs/cli/src` matches only its own module and its test). Contrast `tools:use`, whose `allow` **is** narrowed into the DTS as a string-literal union `sdk/org/libs/core/src/exec/bootstrap.ts:301` · `sdk/org/libs/core/src/typecheck/library-dts.ts:185-188`.
...
- **A "no db" project caches its `null`.** Only `writeProjectTable`'s `onSchemaWrite` drops that cache — a catalog `writeTableSchema` can never bring a live project's db into existence `sdk/org/libs/cli/src/server/session-manager.ts:583-590`.
