...
- `org/docs/cli-api/rest/webhooks.md` — the whole request path: the gateway broker
  ...
  + descriptor-driven + emitter-def adapters), secret resolution, dedupe, threading,
  the full response-code table, and every env var. **Also records a known gap: the GET
...
`org/docs/runtime-globals/events-and-integrations.md` ·
