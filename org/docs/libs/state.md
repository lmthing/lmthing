...
Public entry: `sdk/org/libs/state/src/index.ts:1-19` re-exports the types, `lib/fs`, `lib/pod`, `lib/contexts`, and every hook, plus the named providers `AppProvider`/`ProjectProvider`/`SpaceProvider`.
...
> Corrections to the pre-existing `sdk/org/libs/state/CLAUDE.md`, which is stale: the core types file is `types/project.ts`, **not** `types/studio.ts`; the scoped classes are `ProjectFS`/`SpaceFS`, **not** `UserFS`/`StudioFS`; the middle context is `ProjectContext`/`ProjectProvider`, **not** `StudioContext`/`StudioProvider`; there is **no `{username}/` path segment** (the pod is per-user, so keys are `projectId/spaceId/...`); and the source uses **relative imports**, not the `@/` alias the CLAUDE.md advertises (e.g. `sdk/org/libs/state/src/lib/fs/AppFS.ts:3-7`).
...
- **`instruct.ts`** — `parseAgentInstruct`/`serializeAgentInstruct` for `agents/<slug>/instruct.md`. Its own indentation-aware `parseBlockYaml` handles block lists and block mappings (the shape `scaffoldSpace.ts` writes). `AgentInstruct = {title, knowledge[], functions[], components[], actions[]{id,label,description,tasklist}, defaultAction?, canDelegateTo[], body}`; it accepts the legacy `dependencies` key as a fallback for `canDelegateTo` (`sdk/org/libs/state/src/lib/fs/parsers/instruct.ts:20-263`).
- **`tasklist.ts`** — `parseTasklistTask(filename, content)`/`serializeTasklistTask` and `parseTasklistIndex`/`serializeTasklistIndex`, plus `tasklistTaskFilename(order, id)`. `TasklistTask = {order, id, instruction, input?, output, dependsOn?, optional?, goal?, condition?}`; `order`/`id` come from the `NN-<id>.md` filename (`sdk/org/libs/state/src/lib/fs/parsers/tasklist.ts:89-145`, `255-257`). A **block-YAML** reader preserves the nested `input`/`output` maps the shared inline parser would flatten (`tasklist.ts:10-78`).
...
Three nested providers wire the VFS into React (`sdk/org/libs/state/src/lib/contexts/index.ts:1-8`; mount order `AppProvider` → `ProjectProvider` → `SpaceProvider`):
