...
- **The dynamic plan (soft reminders).** The plan is the checklist the agent maintains with the `todoWrite` system function, persisted to `<spaceDir>/.lmthing/todos.json` (statuses `pending`/`in_progress`/`completed`/`failed`). Two `beforeTurn` reminder providers keep the agent on it, both transient (appended to a single request, never persisted): `openPlanReminder` re-injects every not-`completed` task on **every** top-level request so the agent keeps the statuses current, and `planFirstReminder` nudges the agent to sketch a plan first when none exists yet — latched to fire at most once per session so it does not repeat on each yield-round of turn 1 (`sdk/org/libs/core/src/session/plan-reminders.ts#openPlanReminder`, `#planFirstReminder`; registered in `sdk/org/libs/core/src/session/session.ts#Session` and consumed at `sdk/org/libs/core/src/eval/turn-loop.ts:585-588`). Forks/delegates do not set `beforeTurn`.
