...
- `sessionCapabilities` — full toolkit incl. interactive `ask`, but **no `registerSpace`** (`sdk/org/libs/core/src/exec/capability.ts#sessionCapabilities`);
- `forkCapabilities` — headless (no `ask`), non-orchestrating (no `fork`/`tasklist`); `delegate` only when the task's `canDelegateTo` allows it; write + `registerSpace` follow the role, and read-only roles (`explore`/`plan`) get an `intersectAppCaps`-narrowed grant set (`sdk/org/libs/core/src/exec/capability.ts#forkCapabilities`);
- `delegateCapabilities` — autonomous but a full orchestrator over its own actions/tasklists; no `registerSpace` (`sdk/org/libs/core/src/exec/capability.ts#delegateCapabilities`).

`registerSpace` is therefore a **write-capable-fork grant only** — which is where the shipped callers run (the architect's `register`/`reregister` nodes, both `role: general`). Declaration follows injection: `buildAmbientDts` emits `REGISTER_SPACE_DTS` under the same flag (`sdk/org/libs/core/src/exec/bootstrap.ts:396-400`), so a call from a session or a delegate fails typecheck instead of throwing at runtime.
