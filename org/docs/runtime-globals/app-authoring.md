...
`buildApp()`'s two phases prove *compile*-cleanliness, not *runtime*-correctness, so the `verify` gate layers mechanical scans on top of it — each one a check the compiler cannot make. They live in a HOST-RUN code node (`sdk/org/libs/core/system-spaces/system-appbuilder/tasklists/build_live_project/12-verify.ts`), not in prompt prose: as prose the model had to re-emit the scan on every pass, and in one measured run that was 35% of all build-step errors — a gate that fails to execute reports nothing, which reads as "clean".
