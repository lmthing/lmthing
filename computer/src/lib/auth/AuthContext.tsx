...
async function createVerificationToken(password: string) {
...
async function verifyPassword(password: string, stored: StoredVerification) {
  ...
  try {
    ...
    return { valid: TEXT_DECODER.decode(buf) === VERIFICATION_STRING, key }
  } catch {
    return { valid: false, key }
  }
}

async function saveSession(name: string, key: CryptoKey) {
  try {
    const raw = await crypto.subtle.exportKey('raw', key)
    sessionStorage.setItem(SESSION_KEY, JSON.stringify({ username: name, key: toBase64(new Uint8Array(raw)) }))
  } catch { /* ignore */ }
}
...
async function restoreSession(): Promise<{ username: string; key: CryptoKey } | null> {
  try {
    ...
    if (!username || !encoded || !getStoredVerification(username)) {
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(
    async (inputUsername: string, password: string): Promise<{ success: boolean; error?: string }> => {
      const trimmed = inputUsername.trim()
      if (!trimmed) return { success: false, error: 'Username is required.' }
      if (!password) return { success: false, error: 'Password is required.' }

      const existing = getStoredVerification(trimmed)
      if (existing) {
        ...
        setIsAuthenticated(true)
        await saveSession(trimmed, key)
        return { success: true }
      }

      const { verification, key } = await createVerificationToken(password)
      localStorage.setItem(getStorageKey(trimmed), JSON.stringify(verification))
      setUsername(trimmed)
      setEncryptionKey(key)
      setIsAuthenticated(true)
      await saveSession(trimmed, key)
  ...
  useEffect(() => {
    if (username) localStorage.setItem('lmthing-auth:last-username', username)
  }, [username])
