# Scenario campaign — `07-life-admin` · round 1 (BASELINE + FIRST NEW ACTS)

This is the **first round** for `07-life-admin`. Two goals, in order:

**1. Reach a green baseline.**
- Read `sdk/org/scenarios/07-life-admin/scenario.md`. If `sdk/org/scenarios/07-life-admin/run.mjs` does **not** exist yet, scaffold it from
  `sdk/org/scenarios/_template/run.mjs` and implement the Acts already described in `sdk/org/scenarios/07-life-admin/scenario.md`
  (1:1 with its Acts table), reproducing the literal user flow. If it exists, read it and confirm its
  Acts still match `sdk/org/scenarios/07-life-admin/scenario.md`.
- If `sdk/org/scenarios/07-life-admin/scenario.md` itself is a stub (some scenarios only have a title/idea — check
...
- Choose them from the feature catalog below — capabilities `07-life-admin` does **not** yet cover
  — extending the scenario naturally (same persona, same growing project, realistic drift).
- Add each new Act to `sdk/org/scenarios/07-life-admin/scenario.md`'s Acts table (+ any new user stories / expectations) AND to
  `sdk/org/scenarios/07-life-admin/run.mjs` (1:1), keeping every hardening pattern. Run the FULL scenario (old + new) live,
...
- **Scenario:** `07-life-admin` (`sdk/org/scenarios/07-life-admin`)  ·  **Round:** 1 — BASELINE + FIRST NEW ACTS  ·  **Branch:** `main`
...
Maintain the per-run progress log at **/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/1/07-life-admin/PROGRESS.md**.
