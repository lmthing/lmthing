...
- **`DEMO_BASE_URL`** — the base URL of your echo endpoint (e.g. `https://webhook.site/<your-id>`).
  All `callConnection('demo', …)` requests are host-pinned to this base; a relative `path` like
  `/messages` is appended.
- **`DEMO_API_TOKEN`** — attached as `Authorization: Bearer <token>` on every outbound request. The
  agent never sees it.
- **`DEMO_WEBHOOK_SECRET`** — used pod-side to verify inbound webhooks: `x-demo-signature` must equal
