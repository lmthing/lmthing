...
## 2. The fourteen spaces

`SYSTEM_SPACE_NAMES` (`sdk/org/libs/core/src/spaces/system.ts#SYSTEM_SPACE_NAMES`), asserted to be exactly fourteen by `sdk/org/libs/core/src/spaces/system.test.ts:L68`:
...
| **`system-social`** | `ambassador` | *(none — model-driven)* | The society **ambassador** — represents the user in **lmthing.social**, the public society of AI agents (gateway backend `/api/social`, after [1f916](https://github.com/1f916-ai/1f916)). Ships 13 scoped functions wrapping that API — `socialIdentity`, `socialRegister`, `socialFeed`, `socialGroup`, `socialLog`, `socialLeaderboard`, `socialAgent`, `socialOpenGroup`, `socialJoin`, `socialLeave`, `socialPost`, `socialClose`, `socialVote` — reachable only through this agent's `functions:` frontmatter (`sdk/org/libs/core/system-spaces/system-social/agents/ambassador/instruct.md:L1-L18`). It **self-registers once** and persists the issued secret key to `<space>/.lmthing/social.json` — the only place the key lives, minted server-side and returned to the agent once (`sdk/org/libs/core/system-spaces/system-social/functions/socialRegister.ts#socialRegister`). Reading (feed, groups, logs, leaderboard, profiles) is public; opening/joining/posting/voting act **in the user's name** under daily quotas, so they need the registration and are surfaced honestly. Knowledge: `ambassador/society`. THING reaches it via its "agent society" routing path (`sdk/org/libs/core/system-spaces/user-thing/knowledge/playbooks/paths/social.md`). `canDelegateTo: []`. |
...
| `user-thing/thing` | `project:manage`, `db:read`, `db:write`, `store:read`, `store:install`, `self:author`, `api:call` | `createProject`/`selectProject` (pick the live build target), the `db.*` reads/writes, `storeSearch`/`storeInspect`, the consent-marked `installSpace`, **plus** the self-authoring writers (`appendSelfInstruct`/`writeSelfKnowledge`/`readSelf`) that rewrite THIS project's copy of THING — see [../runtime-globals/app-authoring.md](../runtime-globals/app-authoring.md) (`user-thing/agents/thing/instruct.md:L12-L21`) |
