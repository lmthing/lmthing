...
### Per-project THING overlay — `Session.overlayProjectThing`
After `mergeSystemInto`, `loadMergedSpace` overlays the project's OWN copy of `user-thing` when the session carries a `projectThingDir` opt (`<projectRoot>/spaces/user-thing/`, threaded by the CLI — `sdk/org/libs/cli/src/server/session-manager.ts#SessionManager.projectThingDir`). `Session.overlayProjectThing` (`sdk/org/libs/core/src/session/session.ts#Session.overlayProjectThing`) loads that space and overlays its `thing` agent, tasklists, functions/components **and knowledge domains** onto the merged result — so the RUNNING THING is per-project and the `self:author` writers can specialize it in place (see [../runtime-globals/app-authoring.md](../runtime-globals/app-authoring.md)). Three guardrails: a **defensive load** (a missing/unloadable copy leaves the merge untouched, falling back to the shipped THING — a broken self-edit can never break the session); the same **empty-placeholder guard** as `mergeSystemInto`; and **project-user-space precedence** — a `thing` agent or knowledge domain the project ROOT space itself declares keeps priority over the copy. Knowledge needs the explicit overlay because the system merge is first-wins on domains, so a per-project knowledge edit would otherwise be shadowed by the shipped THING's same-slug domain.
