...
Header: session title, live session cost (`sessionCostUsd + sessionCostInflight`), follow toggle, `ConnectionDot`, `TraceLoader`, Inspect, Report bug, theme toggle, and a restart button `sdk/org/libs/ui/src/chat/app/ChatView.tsx:188-264`. The title prefers the agent-set session title (from `setSessionMeta`, delivered as a `session_meta` trace event) and falls back to `<project|space> · <Agent>` `sdk/org/libs/ui/src/chat/app/ChatView.tsx:168-183`. So a new chat is never title-less, the store seeds `sessionTitle` from the **first user message** (whitespace-collapsed, capped 80 chars) the moment it is sent, as a client placeholder — set by `noteUserMessage` only while no title exists yet, and overridden by a later agent `session_meta` `sdk/org/libs/ui/src/chat/store/session-slice.ts:156`. The header holds the title and nothing live: the chat's **one** status sentence sits at the other end of the view, against the composer `sdk/org/libs/ui/src/chat/app/ChatView.tsx:341` (see [Sub-agent activity](#sub-agent-activity)) ([../runtime-globals/session-and-utils.md](../runtime-globals/session-and-utils.md)).

Blocks are grouped before rendering: a run of non-user blocks becomes one `AssistantTurn` (with the set of contributing node ids), a user block flushes the run `sdk/org/libs/ui/src/chat/app/group-blocks.ts#groupBlocks` · `sdk/org/libs/ui/src/chat/app/ChatView.tsx:310-315`. The grouping is shared: the embedded dock (`ReplChatView`) renders the same `model.blocks` through the same `groupBlocks`. An empty transcript renders `EmptyState` with four suggestion chips that send as ordinary messages `sdk/org/libs/ui/src/chat/app/ChatView.tsx:305-308` · `sdk/org/libs/ui/src/chat/app/EmptyState.tsx#SUGGESTIONS,29-41`.
...
`Composer` is keyed on `activeSessionId` `sdk/org/libs/ui/src/chat/app/ChatView.tsx:351`, so switching sessions remounts it fresh — its draft (`text`/attachments/recording/the `@` dropdown) is local state that nothing otherwise resets when `switchSession` swaps the socket, so without the key a draft typed in one chat stayed in the box after switching to another.

The transcript is a `Prim.Scroll`, a real scrolling region on both targets (not a `Box` with CSS overflow, which Yoga does not support), and follows new output via its `stickToEnd` prop — gated on `follow && atBottom` so it does not yank a reader who has scrolled up back to the bottom — rather than an effect calling the DOM-only `scrollIntoView` `sdk/org/libs/ui/src/chat/app/ChatView.tsx:273-277`. `groupBlocks` is memoized against the block array and its length so a long session does not re-group (and every finished message re-parse its markdown) on every streamed token `sdk/org/libs/ui/src/chat/app/ChatView.tsx#ChatView`.
...
The package also exports a **second, simpler family** of renderers `sdk/org/libs/ui/src/chat/index.ts:5-8`. They are used by `ReplChatView` `sdk/org/libs/ui/src/chat/components/ReplChatView.tsx:127-155` — the connected-session view shared by the embeddable `AgentChatPanel` (the Studio THING dock), the CLI's `--web` DevTools app `sdk/org/libs/cli/src/web/app.tsx:100-112`, and a project-app's own embedded assistant dock (the `chat` view-spec section, which renders `ReplChatView` directly `sdk/org/libs/ui/src/view/sections/chat.tsx#ChatSectionView`) — **not** by the `/chat` `ChatShell`, which renders via `Message` + `renderDescriptor` instead. `ReplChatView` now renders the **same `model.blocks` transcript** the `/chat` surface does, grouped by the shared `groupBlocks` `sdk/org/libs/ui/src/chat/app/group-blocks.ts#groupBlocks` — user turns, markdown assistant turns (via `DisplayBlock`), displays and asks — so the embedded dock reads as the full chat, not a stripped one, while staying decoupled from the global store (`useReplSession` feeds a per-instance model) `sdk/org/libs/ui/src/chat/client/useReplSession.ts#useReplSession`.
...
- `AskBlock` — mirrors `/chat`'s own `Message` branching: a consent descriptor → `ConsentCard`; a catalog form descriptor (`@lmthing/core/ui`'s `isFormDescriptor`, however deeply `<Stack>`/`<Field>`/`<Fieldset>` wraps the actual controls) → the shared `CatalogForm` (above), which resolves a single value when the descriptor is a bare control and an object keyed by field name for a `<Form>`; anything else (a bare string `ask()`, or a display-only descriptor with no recognized field) → a plain text input. Also offers Cancel — `CatalogForm` takes an `onCancel` `AskBlock` passes but `/chat`'s own use of it does not, since an answered ask on the main surface has no way to be un-asked `sdk/org/libs/ui/src/chat/components/AskBlock.tsx#AskBlock`. Before this, `AskBlock` had its own smaller, non-recursive field switch (`textinput`/`select`/`checkbox`, direct children only) that diverged from `CatalogForm` — any of the catalog's ~30 form components (`DatePicker`, `MultiSelect`, …) or a field nested under `<Field>`/`<Stack>` (the shape the appbuilder/architect system spaces actually author) rendered as an empty box with only Submit/Cancel, silently dropping the heading, labels, and every field.
