/**
 * Shape guard for utility-janitor's hooks — a bad trigger string or budget key ships silently
...
const SPACE_ID = 'utility-janitor';
...
test('every hook has a valid shape and stays inside this space', async () => {
  ...
  assert.ok(names.length > 0, 'utility-janitor ships at least the daily scan hook');
...
test('the daily scan triggers the janitor scan action — never apply — with a bounded budget', async () => {
  const hook = await importHook('daily-scan');
  assert.equal(hook.type, 'cron');
  assert.equal(hook.daily, '06:00');
  assert.equal(hook.trigger, 'utility-janitor/janitor#scan');
  assert.ok(!/#apply$/.test(hook.trigger), 'no cron may apply patches unattended — approval is a human act');
  assert.ok(hook.budget?.maxEpisodes > 0 && hook.budget?.maxWallClockMs > 0, 'the cron scan must be budget-bounded');
});
