...
A tasklist node can be an `NN-<id>.ts` file exporting `const node = {…}` metadata + an async `run(ctx, inputs)`. Core **never** imports or executes the module — `loadTasklist` statically AST-extracts the `node` literal and confirms a `run` export (`sdk/org/libs/core/src/spaces/tasklist-load.ts#loadTasklist`, `:161-233`); the metadata goes through the same field validators as md frontmatter (`buildTaskNode`, `:92-159`). The host runs `run(ctx, inputs)` via an injected `codeNodeCtxFactory`; when that factory is **absent**, encountering a code node fails that task as a required-task error (`sdk/org/libs/core/src/tasklist/orchestrator.ts:36-57`, `:200-227`). In a project-scoped CLI session, the worker `ctx` carries `ctx.db`, `ctx.delegate`, connection-gated `ctx.callConnection`, and — for a tasklist that authors files — the typed live-project WRITERS as top-level methods (`ctx.writeProjectTable`/`writeProjectApi`/`writeProjectView`/`writeProjectViewComponent`/…), the SAME host impls the agent nodes hold, so an `implement_*` code node authors deterministically instead of writing model-generated source (`sdk/org/libs/cli/src/server/tasklist-runner.ts#createCodeNodeCtxFactory`, `sdk/org/libs/cli/src/server/session-manager.ts#SessionManager.buildProjectAuthoring`, proxied to the host via `sdk/org/libs/cli/src/app/worker-load.ts#serviceProxy`); every operation is proxied to the host rather than exposing a filesystem. **Every one of those ctx members is ASYNC and must be `await`ed, including the ones whose host impl is synchronous** — the worker receives them as RPC stubs (`authoring[method] = (...args) => rpc('authoring', …)` `sdk/org/libs/cli/src/app/worker-load-entry.ts:L127-L130`), so `ctx.listProjectDir(dir).entries` reads a property off a Promise and is `undefined`. The failure is SILENT: a scan built on it simply finds nothing, the node still resolves, and an empty findings list is indistinguishable from a clean one. A unit test that injects a synchronous mock cannot see it, so drive at least one test with an all-async ctx. A code node's `inputs` mirror exactly what an agent fork would receive (seed-filtered input + upstream outputs + `item`/`index`); it has **no salvage path** — `run` either returns or throws. `forEach` can fan out over a code node (`:210-224`).
