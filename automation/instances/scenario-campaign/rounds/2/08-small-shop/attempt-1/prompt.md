# Scenario campaign — `08-small-shop` · round 2 (ADD ACTS (batch 2))

`08-small-shop` already has a runner and has been extended in earlier rounds. This round: **add
ANOTHER batch of additional Acts and continue from there.** Never regress the existing Acts —
extension is strictly additive.

1. **Orient & resume.** Read `sdk/org/scenarios/08-small-shop/scenario.md` (esp. its Acts table + Actual-results), `sdk/org/scenarios/08-small-shop/run.mjs`,
   and `sdk/org/scenarios/08-small-shop/results/` (report, trace, checkpoint) to see exactly which Acts exist, which passed,
   and any open frontier a prior round recorded. Read this run's `/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/2/08-small-shop/PROGRESS.md` if it exists.
2. **Pick the next batch of Acts (2–4)** from the feature catalog below — capabilities
   `08-small-shop` still does **not** cover — extending the same persona / same growing project
...
3. **Add them to `sdk/org/scenarios/08-small-shop/scenario.md` (Acts table + user stories/expectations) AND `sdk/org/scenarios/08-small-shop/run.mjs` (1:1)**,
...
- **Scenario:** `08-small-shop` (`sdk/org/scenarios/08-small-shop`)  ·  **Round:** 2 — ADD ACTS (batch 2)  ·  **Branch:** `main`
...
(there is no CI/deploy in this loop — see **Target** below). Scenario files live at `sdk/org/scenarios/08-small-shop`;
...
- **Set `SCENARIO_TARGET=local`** in the environment of every harness command (`provision.mjs`,
  `sdk/org/scenarios/08-small-shop/run.mjs`, any probe). With it set, `getUser` returns `{ pod:'http://localhost:8080', token:null }`
...
Extend `08-small-shop` with **additional Acts**, run the **whole scenario end-to-end against the
...
1. **catalog-scout** — READ-ONLY prep: read sdk/org/scenarios/08-small-shop/scenario.md + sdk/org/scenarios/08-small-shop/run.mjs + sdk/org/scenarios/08-small-shop/results and the feature catalog in this prompt; list which catalog capabilities 08-small-shop does NOT yet cover, and propose the next 2–4 Acts (name + what each asserts on the trace/real state). Do NOT provision, open a THING session, or send any turn — analysis only.
...
testing a script. Hold them in `sdk/org/scenarios/08-small-shop/scenario.md` and in every message `sdk/org/scenarios/08-small-shop/run.mjs` sends.
...
- **Every fixture must carry at least one fact/token that appears in NO other fixture**, and an Act
  ...
  assertion worthless.
- Fixtures are **committed and self-contained** under `sdk/org/scenarios/08-small-shop/fixtures/`. Record where each
  came from (source URL) in `fixtures/links.md`.
- **Wire it into BOTH** `scenario.md` (the user flow + the Acts table) and `sdk/org/scenarios/08-small-shop/run.mjs` (uploaded,
...
**Never exercised — pick 2–4 that fit `08-small-shop`'s persona and build them into the story.** A
...
`08-small-shop` — recovered errors stay a metric, unrecovered ones fail the run.
...
`sdk/org/scenarios/08-small-shop/run.mjs`, and if the product cannot do them yet, **implement the missing feature** (§4 fix loop:
...
1. **Bring up the local pod + smoke.** `cd sdk/org/scenarios/harness && SCENARIO_TARGET=local node
   ...
   `SCENARIO_TARGET=local node provision.mjs 08-small-shop` (returns `{ pod:'http://localhost:8080',
...
Maintain the per-run progress log at **/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/2/08-small-shop/PROGRESS.md**.
...
- `sdk/org/scenarios/08-small-shop/scenario.md` has all six sections + the feature checklist + the Acts table. **Round 1: do not
  rewrite the story** — it was authored deliberately; implement it. Later rounds: add the new Acts.
- `sdk/org/scenarios/08-small-shop/run.mjs` **exists and implements `scenario.md` Act for Act (1:1)** — on round 1 you are writing
...
- **From round 2 on: at least one NEW real fixture** sourced from the real web (or Azure-TTS-generated
  for audio), committed under `fixtures/`, uploaded by `sdk/org/scenarios/08-small-shop/run.mjs`, and asserted via a token that
