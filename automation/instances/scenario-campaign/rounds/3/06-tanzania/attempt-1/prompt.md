...
2. **Launch the replay IN THE BACKGROUND and JUDGE STEP-BY-STEP AS EVIDENCE LANDS.** The full replay
   is 20–40 min — FAR longer than one Bash call — so a blocking foreground run is impossible: it hits
   the tool window, and if you then end your turn the runner is orphaned and NOTHING gets judged. The
   generic runner wipes the pod runtime root, starts a clean server, provisions the one project, and
   writes `step-NN.json` + appends `trace.md` **after each step**, so you watch those files instead:

       ART=<ARTIFACT_DIR>   # = the dir holding /home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/3/06-tanzania/PROGRESS.md; evidence + your report live here
       cd sdk/org && SCENARIO_TARGET=local nohup node scenarios/run-yaml.mjs 06-tanzania \
         --fresh-server --out "$ART" > "$ART/run.log" 2>&1 &

   A backgrounded process **survives across your tool calls** (it only dies when your whole SESSION
   ends) — so **keep your session ALIVE by polling** and NEVER conclude while the runner is still
   running and unjudged. `--fresh-server` guarantees the zero-project start on every run.
3. **Poll + judge each step the moment its `step-NN.json` appears in `$ART`** (source of truth — never
   prose): its delegates / yields / errors / reply, the asks, and the real spaces + app tables + DB
   rows after the step, against that step's `expect` and the invariants. Loop like
   `until [ -f "$ART/step-02.json" ] || ! pgrep -f run-yaml >/dev/null; do sleep 20; done` and read
   the newest file each time.
4. **At the FIRST failing step, STOP the replay** — `pkill -f "run-yaml.mjs 06-tanzania"` — and go
   to attribute + fix. Do NOT wait for the remaining steps: they run on state your fix will change
   anyway. If EVERY step passes (the runner exits, `run.log` ends with `played 18/18`), the scenario
   is fully green → extend.
5. **The verify-rerun** (after a fix): rebuild if you touched code (`pnpm --filter @lmthing/core...
   build` for a system-space or core fix — the fresh server adopts the rebuilt `dist/system-spaces`
   tree on boot), then replay the same way but only THROUGH the previously-failed step:

       cd sdk/org && SCENARIO_TARGET=local nohup node scenarios/run-yaml.mjs 06-tanzania \
         --fresh-server --through <N> --out "$ART" > "$ART/run.log" 2>&1 &

   Poll `step-<N>.json`, confirm it (and every earlier step) now passes, then stop. `--plan`
   dry-prints the step list without touching a pod.
...
## The report (write it as `report.md` in the same directory as your progress log `/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/3/06-tanzania/PROGRESS.md`)

There is NO per-scenario `results/` dir and NO `run.mjs` — a scenario is just `scenario.yaml` +
`fixtures/`, played by the generic runner. Your report lives beside `/home/vasilis/LMTHING/lmthing/automation/instances/scenario-campaign/rounds/3/06-tanzania/PROGRESS.md` in the round's
