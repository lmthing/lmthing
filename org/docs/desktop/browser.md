...
**The WebSocket URL is resolved in Rust**, not by the renderer. Chromium's `/json/version` sends no `Access-Control-Allow-Origin`, so fetching it from the webview is blocked by CORS — in the packaged app as much as in a browser. Chromium writes both the port and the browser target's socket path into `DevToolsActivePort`, so there is no HTTP request to make (`sdk/org/apps/desktop/src-tauri/src/browser.rs:L209-L224`).
