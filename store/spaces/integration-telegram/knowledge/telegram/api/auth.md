...
- **`INTEGRATION_TELEGRAM_BOT_TOKEN`** — the bot token from BotFather (looks like `123456789:AA...`). It is used
  for every outbound call.
- **`INTEGRATION_TELEGRAM_WEBHOOK_SECRET`** — a random string the user chooses and registers with `setWebhook`.
...
1. Reads the user's `INTEGRATION_TELEGRAM_BOT_TOKEN` from the pod env.
...
If the token is missing, `callConnection` throws ("not configured — set INTEGRATION_TELEGRAM_BOT_TOKEN in
...
the user runs `setWebhook` with `secret_token=<INTEGRATION_TELEGRAM_WEBHOOK_SECRET>`, Telegram sends that value
back on every webhook in the `X-Telegram-Bot-Api-Secret-Token` header. The pod compares it against
the stored `INTEGRATION_TELEGRAM_WEBHOOK_SECRET` and rejects mismatches — so the agent can trust that any update
