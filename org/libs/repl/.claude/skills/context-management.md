...
## `{{TASKS}}` Block Generation

On every `stop()` call, the host appends a `{{TASKS}}` section to the injected user message showing the current state of all active tasklists. This gives the agent visibility into task progress, ready work, and blockers.

### Generation

The `{{TASKS}}` block is generated by `generateTasksBlock(tasklistsState)` alongside `generateScopeTable(sandbox)`. It is called during the stop sequencing (step 2-4) and appended to the `← stop { ... }` user message body.
...
{{TASKS}}
┌ build_report ─────────────────────────────────────────────────┐
│ ✓ fetch_sources    → { urls: ["https://..."] }                │
│ ✓ fetch_data       → { rows: 150 }                           │
│ ◎ analyze          (ready — deps satisfied)                   │
│ ○ write_report     (blocked — waiting on: analyze)            │
└───────────────────────────────────────────────────────────────┘
```
...
|--------|-------|---------|
| `✓` | completed | Shows truncated output |
| `✗` | failed | Shows error message |
| `⊘` | skipped | Shows reason |
| `◉` | running | Shows progress if available |
| `◎` | ready | Agent should work on these next |
| `○` | pending | Shows blocking deps |

### Decay tiers for `{{TASKS}}`

| Distance | Treatment |
|----------|-----------|
| 0-2 turns | Full table with outputs |
| 3-5 turns | Status symbols + task names only (no outputs) |
| 6+ turns | Removed |

### Placement rules

- `{{TASKS}}` is appended to the `← stop` user message, **not** the system prompt
- Also appears in incomplete task reminder messages (the `⚠ [system]` injection)
- **NOT** part of `{{SCOPE}}` — separate concern (variables vs. plan progress)
