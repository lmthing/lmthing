...
> UNVERIFIED: No shipped tasklist `index.md` declares a `connections:` key — searched every `*/tasklists/*/index.md` under `store/` and `sdk/org/.lmthing/`. The behavior above is grounded only in the loader/runner code, not in an on-disk example.
