...
`pnpm thing` runs `sdk/org/scripts/thing-dev.mjs`: `pnpm --filter @lmthing/core dev` AND `pnpm --filter @lmthing/cli dev` (both `tsup --watch`, `sdk/org/libs/core/package.json`/`sdk/org/libs/cli/package.json:19`) rebuild their dist on every source change (`:64-76`), then `lmthing serve --port $THING_PORT` is spawned with `LM_DEV_WEB=<sdk/org/apps/web>` and auto-restarted whenever `libs/cli/dist` OR `libs/core/dist` changes (`:82-126`). **Both watchers matter**: the CLI resolves `@lmthing/core` through its built dist, not source (`apps/web`'s Vite config aliases the UI libs to source, `@lmthing/cli` does not) — without the core watcher, a core-only source change (a new capability, a session/space-loading fix) keeps `serve` running against whatever `libs/core/dist` was last built until something unrelated bounces it. That gap is exactly what shipped `Agent "thing" not found` in every fresh project after the `self:author` capability landed in `capabilities.ts` while `libs/core/dist` was three weeks stale — the production compute image was never affected (`devops/argocd/compute/Dockerfile` runs `tsup` for `libs/core` fresh on every image build, `:83`), only this local dev loop, which used to rebuild `cli` alone. That env var makes the CLI start Vite **in-process** (`middlewareMode: true`, HMR attached to the CLI's own HTTP server) so the web app, `/api` and the agent WS are all served from **one** port — `http://localhost:8080` by default, overridable with `THING_PORT` (`sdk/org/scripts/thing-dev.mjs#SERVE_PORT`; `sdk/org/libs/cli/src/server/serve.ts:389-393`; `sdk/org/libs/cli/src/server/dev-web.ts:17`, `:27-36`; the CLI's own fallback port is also 8080, `sdk/org/libs/cli/src/cli/bin.ts:337`). This is the only local mode that gives the SPA a working backend without the gateway.
...
`pnpm agents` (`package.json:10`) brings up the coding-agent fleet in a dedicated herdr workspace named `fleet`, driven by [`devops/scripts/start-agent-fleet.sh`](../../../devops/scripts/start-agent-fleet.sh). A fresh fleet is a **single** pane — the `orchestrator` (claude opus, `--dangerously-skip-permissions`), seeded with [`.claude/agent-fleet.md`](../../../.claude/agent-fleet.md) as standing policy (`devops/scripts/start-agent-fleet.sh:120-125`, `:128-130`); no subagent panes exist yet (`devops/scripts/start-agent-fleet.sh:5-8`). The orchestrator spawns each one on demand with [`devops/scripts/spawn-agent.sh`](../../../devops/scripts/spawn-agent.sh) `<name>`, which splits a fresh pane, runs `herdr agent start`, and prints `<name> <pane-id>` on its final stdout line (`devops/scripts/spawn-agent.sh:107-116`). Requirements: the herdr TUI is already running (the script probes `herdr workspace list`; there is no detached server start — `devops/scripts/start-agent-fleet.sh:41-45`) and `herdr`, `claude`, `pi`, and `agy` are on `PATH` (`devops/scripts/start-agent-fleet.sh:36-39`; the agent kinds the roster launches, `devops/scripts/spawn-agent.sh:24-30`; the spawn helper repeats the same preflight, `devops/scripts/spawn-agent.sh:54-62`).

The orchestrator is restartable, and the idempotency check is on the **agent**, not the workspace: a live `orchestrator` is left alone (`devops/scripts/start-agent-fleet.sh:66-71`), while a `fleet` workspace that outlived its orchestrator gets a new one without disturbing the subagent panes — the script reuses a pane that is back at a free shell (exporting `AGENT_BROWSER_SESSION=orchestrator` into it) or splits one when every pane is occupied (`devops/scripts/start-agent-fleet.sh:96-117`). So killing the orchestrator pane and re-running `pnpm agents` is a clean restart; `pnpm agents --restart` (`devops/scripts/start-agent-fleet.sh:24-31`) does the kill for you — closing the old pane and waiting for herdr to release the name before reusing it (`devops/scripts/start-agent-fleet.sh:76-86`) — and refuses to run from the orchestrator's own pane, which it would close out from under itself (`devops/scripts/start-agent-fleet.sh:72-75`). The seed prompt tells a restarted orchestrator to adopt the agents already in `herdr agent list` rather than respawn them (`devops/scripts/start-agent-fleet.sh:129`).

The spawn helper owns the roster — the name → agent kind + model args mapping (`devops/scripts/spawn-agent.sh:14`, `:22-33`): `claudez` (claude via `~/.claude/zai-settings.json`), `pi-glm` (pi on `zai/glm-5.3`), `pi-glm-flash` (pi on `zai/glm-5.3-flash`), `pi-luna` (pi on `azure-responses/gpt-5.6-luna`), `pi-terra` (pi on `azure-responses/gpt-5.6-terra`), `pi-deepseek-flash` (pi on `azure-chat/DeepSeek-V4-Flash-0731`) and `agy` (the Antigravity CLI on `gemini-3.7-flash --effort medium`) (`devops/scripts/spawn-agent.sh:24-30`). Spawning is idempotent — an already-alive agent is a no-op that just prints its existing pane id (`devops/scripts/spawn-agent.sh:80-85`) — and if `herdr agent start` fails the helper closes the pane it just split (`devops/scripts/spawn-agent.sh:110-114`). Standing policy, from the guide seeded above: the orchestrator achieves tasks by delegating to the seven roster agents over the `herdr` CLI, and never answers a blocked approval dialog itself.

Every pane the helper splits is given `AGENT_BROWSER_SESSION=<name>`, so each agent drives its **own** isolated `agent-browser` browser instead of sharing the single default instance (`devops/scripts/spawn-agent.sh:104-108`). `agent-browser` itself is a globally installed CLI on `PATH`, not a repo dependency — it is how the `pi`/`agy` agents browse at all, since they have no MCP client and cannot reach the orchestrator's `chrome-devtools` MCP server. Its command surface and traps are part of the orchestrator's standing policy ([`.claude/agent-fleet.md`](../../../.claude/agent-fleet.md)).
